#!/usr/bin/env bash
# masterplan-self-host-audit.sh — developer-only audit checks for the masterplan repo.
#
...
if [[ "${ORIGIN_URL}" != *masterplan* ]]; then
  echo "Skipping: this audit only runs inside the masterplan repo (origin: ${ORIGIN_URL:-unset})"
  exit 0
fi
...
check_drift() {
  ...
  if [[ -f "${plugin_registry}" ]] && grep -q "masterplan" "${plugin_registry}" 2>/dev/null; then
    plugin_registered=1
  fi
...
check_codex_packaging() {
  ...
  if [[ "${marketplace_name}" != "rasatpetabit-masterplan" ]]; then
    echo "⚠️  .agents/plugins/marketplace.json — marketplace name is '${marketplace_name}', expected 'rasatpetabit-masterplan'"
    EXIT=1
  fi
