...
> dedicated agents (`agents/*.md`), the fabric dispatch path (`the fabric wave dispatcher (`mp dispatch-wave`)`), and
...
5. **CD-7 commit discipline.** Each durable change = an `mp` write (atomic) FOLLOWED BY a `git
   ...
   is safe (`state.yml` leads, resume re-commits). Wave members (agents / the fabric path) return
...
`the fabric wave dispatcher (`mp dispatch-wave`)` (L2) runs **exactly one wave per launch**; the LAUNCH half now lives
...
returns the `dispatch_fabric` op the shell executes (`cd "<WT>"` → fabric wave dispatch →
...
2. **Act on the returned JSON.**
   ...
   - Otherwise **narrate tersely** — at most a 1–2 line wave summary (what completed / what's
     ...
     `run_skill finish`). On a goals-enabled bundle, fold a **one-line goals reminder** into that terse summary — the run's frozen goals (from `goals.md`) are the finish-time bar (§2c `run_goal_check`), so surfacing them each wave keeps them from being forgotten mid-run.

## 2b — Parallel-plan dispatch + completion (the planning L1↔L2 seam)

`the fabric plan fan-out (`mp dispatch-plan`)` (L2) fans out **one `mp-subsystem-planner` per subsystem** in a single
...
2. **Phase-1 plan marker.** `mp set-active-run --state=<path> --kind=plan` — a planning marker (no
   ...
   re-dispatch.
3. **Re-enter the §2 loop.** `mp continue` now returns the `dispatch_fabric workflow:'plan'` op —
   per the §2 table: launch `the fabric plan fan-out (`mp dispatch-plan`)` in the background with
   `args = { subsystems:<step 1>, specPath:<spec_path>, repoRoot:<repo> }`, close to await the completion
...
5. **Review.** Dispatch `agents/mp-plan-reviewer` against `plan.md` / `plan.index.json` / `spec.md`
   ...
   - **PASS** → `mp clear-active-run`; satisfy the **plan gate** first (§3b — `mp load-plan` exits 3 with a
     ...
     then re-enter the §2 loop (step 4 — `mp continue` returns the wave-1 `dispatch_fabric` op,
...
| `run_adversary_review` `{base, head, wt, digest_path, fallback_reviewers, fallback_reason?}` | The whole-branch cross-vendor adversary review — network, stays shell-side. Run it harness-native over the branch diff: the **adversary class** (`breaker` role, frontier lane; adversarial panel for cross-vendor coverage — resolved from the checked-in `policy/workflow-map.json`), foreground from WT, no local review engine. On **a real PRIMARY review**: `Write` a brief digest (count + top findings, not the raw dump) to `op.digest_path` (absolute-MAIN, §2e¶1; the Write tool is not shell-evaluated, so arbitrary review bytes are safe — never interpolate the digest into a shell word), then `--review-done --review-count=<n> --review-base=<op.base> --review-digest-file=<op.digest_path> --review-reviewer=<primary model ref or 'adversary'>` (no fallback reason) — finish-step emits the durable `adversary_review` event (its `summary` is the audit signal that DOES match `\b(codex\|adversary)\s+review\b`; `data.sha/base/count` are the quote-safe machine scalars the re-entry guard keys on; `data.reviewer` names who reviewed; `note` carries the digest verbatim for gate rehydration). **Fallback (review-fallback):** when the primary is NOT a success — a non-zero exit, a **refused or blocked launch** (e.g. the fleet review circuit breaker refusing `breaker`/`*-adversarial-reviewer` names past a per-file round cap), the review unavailable/empty, a failed harness spawn — try each entry of `op.fallback_reviewers` **in order, one attempt each**: dispatch the read-only **`mp-fallback-reviewer`** agent — **under the adversary class** (task class `adversary`; the agent declares `preset: breaker`), so the harness's spawn guard authorizes the override — with THAT entry as the harness-native model override, over the same branch diff (inline diff preferred — the fallback agent is read-only). If the harness cannot run a given model, that is a failed attempt; move on. The first attempt that produces a real review wins: `Write` its digest to `op.digest_path`, then `--review-done --review-count=<n> --review-base=<op.base> --review-digest-file=<op.digest_path> --review-reviewer=<that model ref> --review-fallback-reason="<why the primary failed>"`. **Only when the primary AND every fallback fail**: `--review-skipped --review-reason="<primary reason>; fallback <ref>: <reason>; …"`. `op.fallback_reviewers` is resolved in lib — never re-derive routing policy in the shell: the adversary class `chain`, primary excluded, de-duplicated (the class PANEL is never consulted — a panel member outside the chain would be refused by the class's spawn guard); the `.masterplan.yaml` key `adversary_review_fallback` replaces that list outright (an entry outside the chain is refused fail-closed — it could never run — while a policy outage passes the configured list through, the chain being unknown), and `adversary_review_fallback: off` disables the fallback (skip-on-failure as before the feature); an unavailable routing policy yields an empty list and `op.fallback_reason` says why — in every empty case the gate behaves exactly as it does with no fallback. Fail-soft, never wedge finish, unchanged: finish-step writes the sha-keyed skip event whose hyphenated "adversary-review … skipped" summary deliberately does NOT match the audit's `\b(codex\|adversary)\s+review\b`, so a degraded finish still trips `adversary_review_configured_but_zero_invocations` — correct. **Governance caveat:** `mp-fallback-reviewer` is deliberately named outside the fleet review circuit breaker's match, so a finish-gate fallback review does not count against the per-file review-round cap; it stays bounded to one review per HEAD by the existing re-entry guard, and `adversary_review_fallback: off` turns it off. Residual window: a death between the reviewer's completion and the re-invoke leaves no record at HEAD, so resume re-runs the review — harmless and idempotent at an unchanged tree. |
| `ask` `ask:'gate'` `gate:'branch_finish'` `{head, branch, base, dispositions, review}` | First **check for an open PR** (the §3 PR check: `gh pr list --head "<op.branch>" … \| mp pr-summary`). AUQ labelled with `op.base`: `Merge to <base> locally (Recommended)` · `Push and open a PR` · `Keep branch + worktree as-is` · `Discard everything` (typed "discard" required). If the check found a PR (`hasPr`), relabel the second option → `View / merge open PR #<n> (mergeable: <yes\|no\|unknown>)` — same `pr` choice; its resolution is a no-op push surfacing the existing PR's URL, never a second one. Fold `op.review` (`{present, digest, count, base, reviewer, fallback}`, rehydrated from the durable event — the live in-context digest does not survive compaction, the event does) into the AUQ when present, and name the reviewer in the review line: `reviewed by <reviewer>`, plus `via fallback (<fallback.reason>)` when the review came from a fallback reviewer — the operator must be able to see that the primary adversary lane did not run, and why, before choosing a disposition. This AUQ is the turn-close. Resolution = re-invoke with `--choice=<merge\|pr\|keep\|discard>` (add `--removal-force` only for an intended-dirty teardown): finish-step runs the disposition transaction (§2e¶7) and archives — except `pr`, which first returns the `shell push_pr` op (two-phase: archive happens only after `--choice=pr --pushed`). A free-text / "not ready" answer holds the gate and chats (§2 `ask:'gate'` rule) — the "not done yet" escape, nothing archives. |
...
- "What should I do next?" / "dispatch the next wave?" — between successful steps you **auto-proceed**:
  ...
  `recover_wave`, `dispatch_fabric`, or no-Workflow `dispatch_fabric` op is an
...
3. **cwd is a WRITE-ONLY signal to the about-to-launch agents — never read it back.** Subagents (both
   ...
   is explicit-`-C`, ¶2). `dispatch-wave`'s "your launch cwd IS the target repo (${repoRoot})"
   holds iff we cd'd to WT and pass `repoRoot:<WT>` in the launch args.

4. **Create-or-reuse — internalized in `mp continue`.** The worktree existence check (`branchExists` +
...
5. **Global orphan sweep — `mp sweep`, once per SESSION at first §2 entry (a dead run can't clean up
...
| `status` | Read-only: `mp decide` (no writes) + a one-screen situation report from `state.yml`. **PR-aware** (PR check ↓): if the branch has an open PR, append the `↪ Open PR #<n> …` line. **Goals-aware:** on a goals-enabled bundle, use `mp goals-status --state=<path>` (a separate read-only subcommand) for the anti-forgetting surface — the frozen `goals.md` goals, tombstones, and the frozen/amended hash lineage — since the base `status` render carries only `slug / status / phase / tasks / refs / other_runs`. |
...
**PR check (`status` · `next` · `clean` — never auto-merge).** These three verbs check
...
3. **Serial path.** Dispatch the `mp-planner` agent against the approved `spec.md` → it writes
   ...
   **`mp load-plan --state=<path> --plan-index=<plan_index_path>`**
   ...
   wave-1 `dispatch_fabric` op, creating-or-reusing the worktree itself per §2e¶4; the single
...
**Optional generated images (`state.render.images`) — the plan→execute seam step.** After the plan gate resolves and `mp load-plan` has materialized `state.tasks` + advanced `phase→execute` (BOTH the serial §3a path above and the parallel §2b PASS path converge here), if — and only if — the bundle's `state.render.images` is `on`, the **SHELL** (never `mp` — `mp` never touches the network or a secret) dispatches image generation through the **skynet gateway image lane** for the slot-name convention `assets/{hero,wave-<n>}.png` (one `hero.png` plus one `wave-<n>.png` per wave), writes the resulting PNGs into the bundle's assets dir `<MAIN>/docs/masterplan/<slug>/assets/`, then re-runs `mp render-plan --state=<path>` so the fresh `plan.html` embeds them. **No API key, the lane down, or the flag `off` → SKIP silently**: the render is already complete regardless (embedding is **by-presence** — `render-plan` never consults `state.render.images`; the flag gates GENERATION only, so assets already on disk keep rendering after the flag is turned off, and deleting `assets/` un-embeds them). This step is what makes the flag real: without it the `mp set-render-config --images=on` and `mp seed --render-images=on` setters are inert. It is purely additive at the seam — never a gate, never blocking; a failed or skipped generation leaves the deterministic, secret-free render untouched.
