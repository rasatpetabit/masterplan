...
4. **The loop.** `mp continue --state=<MAIN>/docs/masterplan/<slug>/state.yml [--alive|--dead]
   ...
   | `dispatch_fabric` | The `state.dispatch.fabric` strangler path — the agent-dispatch broker executes the wave. Run `mp dispatch-wave --state=<path>` (ONE deterministic command — dispatch AND record complete inside it; pass `--codex-suppressed` when §0 host-detect reported a Codex host — the prepare inputs are frozen into the record's `routing_inputs` and reused by retries, so descriptors never drift across attempts; Guard-D ownership is acquired inside the command — a blocked owner exits loudly with nothing dispatched): it re-derives the routed tasks from the same `prepareWave` seam, builds one adsp work item per task (`buildWorkItem`), drives the broker's `dispatch_fanout` through one `agent-dispatch serve-mcp` process (coord job opened/closed in a `finally`, paired even on failure), then runs the SAME `record-result` transaction — digests carry adsp-v1.1 `dispatch` provenance (`worker` on success; degradations emit `dispatch_degraded` events). Idempotent on the wave-dispatch record `(run_id, wave, 'dispatch_fabric')` persisted in the bundle BEFORE the broker call: a re-invoke after an accepted-but-unobserved dispatch returns `outcome:'reused'` with the existing record and does NOT re-dispatch — surface a stuck `status:'pending'` record via AUQ (`mp dispatch-wave --takeover` supersedes a confirmed-dead attempt); a `'dispatched'` record re-drives record-result from the stored digests without touching the broker. Act on the embedded record outcome per §2a step 2. NO promote, NO probe — there is no background task; a crash re-emits this op on the next `continue`. |
...
| `ask` `ask:'gate'` `gate:'goals_unmet'` `{unmet, assessment, base, head, wt, manual?}` | The durable goals gate — the run's frozen goals were not all met by the delivered work. Surface `op.unmet` (the unmet goals + the assessor's evidence, or, in `manual` mode, the goals awaiting the operator's own verdict) via AUQ: **Fix & continue** → `mp clear-gate`, close (fix code + commit, then resume → the goal check re-runs fresh over the new HEAD and re-opens the gate if still unmet) · **Accept waiver** → `--goals-waived --waiver-reason="<tight reason>"` (the reviewed override — records the SHA AND clears the gate so a re-entry doesn't re-loop, and appends the durable waiver event) · **Abort finish** → `mp clear-gate`, close (the run stays resumable; nothing archived). In `manual` mode the same AUQ additionally carries the met/unmet verdict itself. A free-text / "not ready" answer holds the gate and chats (§2 `ask:'gate'` rule). NEVER auto-waive regardless of autonomy. |
...
| `status` | Read-only: `mp decide` (no writes) + a one-screen situation report from `state.yml`. **PR-aware** (PR probe ↓): if the branch has an open PR, append the `↪ Open PR #<n> …` line. **Goals-aware:** on a goals-enabled bundle `mp status` renders a **goals block** (the frozen `goals.md` goals + any recorded goal-check verdict) — the anti-forgetting surface so the run's goals stay visible between waves. |
