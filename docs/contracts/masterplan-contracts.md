...
algorithm: |
  ...
  For each check, run the algorithm enumerated in parts/doctor.md's per-check row:
    ...
    #53 cc2_banner_compaction_resume_compliance: Read the active bundle's events.jsonl (schema_version >= "5.1"); measure cc2_banner_emitted ratio over the latest 20 compaction-resume / invoked_skills-reinjection turns only (exclude fresh invocations); warn <0.8, error <0.5. Run per Check #53's algorithm in parts/doctor.md.
  Aggregate all per-check findings into a single violations[] array.
return_shape: |
  contract_id: "doctor.repo_scoped.schema_v1"
  checks_processed: [26, 30, 31, 36, 39, 44, 46, 47, 48, 49, 50, 51, 52, 53]
...
algorithm: |
  ...
  2. Extract: mode, repo_role, verification_ceiling, in_scope_paths, out_of_scope_repos,
     evidence list, yocto_ownership, gate_selection, interview_depth.
...
return_shape: |
  ...
  out_of_scope_repos: [...]
  evidence: [...]
  gate_selection: "<gate or null>"
  interview_depth: {complexity, target_question_count}
