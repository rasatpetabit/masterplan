...
### 2026-09-06 — Operator authorized moving the branch base: merge main into masterplan/intent-to-completion
The accepted scope amendment fixed the branch code base at 1f2741f. Implementation of tasks 49-59 cannot proceed against it: the branch worktree carries spec.md 6111aaf4, the pre-amendment document the approved amendment replaced, and lacks INTENT.md entirely, which §6.3 would report as run-wide drift. Asked how the branch should reach the approved spec, the operator chose 'Merge main into the branch' over rebase, over reading the spec from main, and over stopping. This supersedes the 1f2741f base boundary and nothing else in the accepted scope: no push, no deploy, no release evidence, and the 48 completed task records stay intact. Branch had 18 commits not in main; main had 71 not on the branch.
