...
Planning-time task amendment applied through mp amend-tasks / amend-plan with no other content change.

### 2026-09-03 — Doctor --only=<check-id> option assigned to config-knobs.env-reads (owner of bin/doctor.mjs)
Spec section 7.1 checks and the G6 evidence invoke node bin/doctor.mjs --only=<check-id>, which the installed doctor does not support; no task owned it. Found at wave-0 dispatch; amended before any wave-2 launch.
