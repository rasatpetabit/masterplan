...
## G6: v10.0.0 is released, tagged, pushed, and installed into both running surfaces before this run's finish begins
signal: command
evidence: Live at the goal check, both surfaces: `node bin/install-pi.mjs --check` prints check_ok with version 10.0.0; `git rev-parse -q --verify refs/tags/v10.0.0` exits 0 at the branch tip; `git ls-remote --exit-code --tags origin refs/tags/v10.0.0` exits 0; `node bin/doctor.mjs --only=plugin-registry-drift` exits 0 reporting v10.0.0 installed in the Claude plugin cache; bootstrap_step events on this bundle record each command, including the rehearsal, the pre-finish merge to main, the push, and the operator's plugin update.
