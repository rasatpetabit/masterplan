...
1. **The bootstrap stage — not a plan wave: a serial, operator-gated stage (risky-action AUQs)
   ...
   Three more typed events carry the walk's honesty contracts, each append-only and validated at
   append time: `bootstrap_refusal {pass, step, refusals, evidence}` — the record of a
   post-publication precondition refusal, which exists only while the refusal is live (the
   step's `pre()` is re-run at record time; a step that does not currently refuse cannot be
   recorded), is limited to the pass's CURRENT next executable post-push step — `gate`
   excluded (its refusals are arm-time preconditions with their own rows) and cascade refusals
   (a later step refusing only because an earlier step never ran) explicitly NOT corrective
   triggers — and is never a `bootstrap_step` receipt (no command ran); `main_advance_reconciled
   {sha, prior_expected, paths, author}` — the evidence-bound reconciliation of an origin/main
   advance that is a single non-merge doc-only commit touching neither the bundle directory nor
   any path the branch changed (path-rule-verified, else the foreign-commit row); and
   `receipt_correction {targets, note}` — an append-only disclosure against historical receipts
   whose targets are never rewritten (§10.3's refusal row names the first two, the disclosure the third).
...
| post-push step arm REFUSES (a precondition refusal — no command ran): the branch tip moved past the published release tip (post-tag corrective work) or main moved past the last recorded base (an unrecorded push) | the tag is public and frozen: **corrective release**, opened on the refusal itself. `bootstrap record-refusal --step=<step>` records a typed `bootstrap_refusal {pass, step, refusals, evidence}` — the refusal re-derived live at record time, never a `bootstrap_step` receipt (no command ran; a step that does not currently refuse cannot be recorded); the step must be the pass's CURRENT next executable post-push step (`gate` excluded; cascade refusals are not corrective triggers). The corrective pass opens on it under the red-tag shape: predecessor complete through push, the refusal causally after its push record, every missing step post-push. An unrecorded main advance reconciles first when it is a single non-merge doc-only commit (every changed path under docs/) touching neither the bundle directory nor any path the branch changed: `bootstrap reconcile-advance --sha=<origin/main tip>` records `main_advance_reconciled` and the pr_merge expected base then resolves from it; anything else is the foreign-commit row (stop, inspect, revert or reconcile by hand — never widen the audit to admit it) |
