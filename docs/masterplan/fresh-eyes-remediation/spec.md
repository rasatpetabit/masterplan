...
| Item | Action |
|---|---|
| A1 goal-gate flags dead | **Repair.** Wire `bin/masterplan.mjs` to the engine's existing vocabulary: `--goal-check=<failed>` / `--goals-choice=<fix\|waiver\|abort>` (per `lib/finish-step.mjs`, `GOALS_CHOICES`). Rewrite `commands/masterplan.md` §finish to teach these flags (replacing the unimplemented `--goals-met/--goals-unmet/--manual-verdict/--goals-waived/--waiver-reason`). The stray uncommitted marketplace patch adding these two flags becomes obsolete — the repo is the source of truth. |
| A2 default class unresolvable | **Repair.** Repoint `FABRIC_DEFAULT_CLASS` (`lib/wave.mjs`) from absent `masterplan-implementation` to the existing class `planned-execution`. Fix the 5 test files pinning the dead name. Registering a dedicated class in the fleet-owned workflow-map source is a fleet-side follow-up, out of scope. |
...
| A7 unknown flags silently ignored | **Repair.** `parseArgs` fails closed (doctor-style exit 2) on unrecognized `--flags` for all `mp` verbs. Compatibility-scan every documented invocation (`commands/masterplan.md`, `docs/verbs.md`, skills, repo scripts) before landing; dedicated tests. Highest-risk item of the bundle. |
| A8 workspace-root drift inert off-fleet | **Repair.** Derive the workspace root from git (toplevel) instead of the hardcoded `/^\/srv\/dev$/` + wrong-depth derivation; fix the consumption at `lib/wave-commit.mjs`. |
...
| Item | Action |
|---|---|
| B1 CI Doctor red | **Repair.** Root fix is A1's mechanism; then record a `goal_check` receipt for the archived `blocked-task-injection` bundle via `mp` (CD-7 — never hand-write state). Doctor exits 0; CI unblocks. |
| B2 goals-hash mismatches | **Repair each.** Re-freeze the 4 drifted bundles (`dispatch-subagent-reconcile`, `fabric-default-dual-reg`, `planf3-ideas`, `simplify-dedup-2`) via `mp goals-load`/amend; address the scalar-cap 245>200 on `simplify-dedup-2`. |
...
| A1 goal-gate vocabulary | Wire bin to engine vocabulary (`--goal-check`/`--goals-choice`); rewrite prompt to match | Engine vocab already exists in state/ops; minimal diff; supersedes the stray marketplace patch | assumed |
| A2 default class | Repoint to existing `planned-execution`; fix 5 test files | Hand-editing the fleet-generated workflow-map would drift; fleet registration is a separate follow-up | assumed |
...
| Adversarial design review | Masterplan's spec gate (cross-vendor pass over spec.md + goals.md at `set-phase plan`) is the governing review | Sequencer §3b owns the review route; no second pre-approval panel stacked on top | assumed |
