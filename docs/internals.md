...
│   ├── doctor.md                   # all 41 doctor checks (#1-#41)
...
**Contract registry:** `commands/masterplan-contracts.md` — one entry per contract, with `purpose`, `algorithm`, and `return_shape`. As of v5.8.0 the registry holds 9 contracts: 4 original (`import.convert_v1`, `doctor.schema_v2`, `doctor.repo_scoped.schema_v1`, `retro.source_gather_v1`), `codex.review_wave_member_v1` for the per-member wave-end review path, and 4 freeform-site contracts that backfilled previously-uncontracted lifecycle dispatches in `parts/step-c.md` (`step-c.eligibility_cache_build_v1`, `step-c.wave_implementer_v1`, `step-c.codex_exec_v1`, `step-c.codex_review_serial_v1`).

**Lint check:** `bin/masterplan-self-host-audit.sh --brief-style` scans `commands/masterplan.md`, `parts/step-c.md`, and `parts/doctor.md` for dispatch blocks tagged with lifecycle `DISPATCH-SITE` values that lack `contract_id` within 30 lines (Pattern D), and for outcome-only language patterns within dispatch contexts (router file only — v5 phase files predeclare contract IDs at the site). The v5 `DISPATCH-SITE: step-c.md:<label>` and `DISPATCH-SITE: doctor.md:<label>` conventions are recognized alongside the legacy `DISPATCH-SITE: Step B0 …` / `Step D …` labels in `commands/masterplan.md`. Lines containing backticks are skipped to avoid matching the convention's own documentation in prose. Exit 0 if clean.
...
| `subagents.jsonl` | Per-Agent-dispatch telemetry emitted by the Stop hook; local-only and ignored before write. Per-record fields include `subagent_return_bytes` (v5.8.0+) — the byte size of the subagent's return payload as ingested by the orchestrator, enabling context-pollution detection via the `subagent_return_oversized` failure class (threshold 5120 bytes). |
...
- **Brainstorming starts with an intent anchor and adaptive interview.** Step B1 must persist `brainstorm_anchor`, `interview_depth`, and a `brainstorm_anchor_resolved` event before invoking the upstream brainstorming skill. Audit/review, deferred-task, and cross-repo prompts get structured gates instead of broad feature-idea funnels.
...
**Asymmetric review enforcement (v5.8.0).** Both the serial Step 4b path and the new wave-mode 4b path consult `dispatched_by` on the completion event before dispatching review. When `dispatched_by ∈ {codex, codex+claude-fixup}`, review is skipped with `decision_source: codex-produced (asymmetric rule)` and a `review→SKIP(codex-produced)` event is emitted. This codifies the principle that Codex shouldn't review its own work, applied uniformly across serial and wave routing.
...
3. **Step C 4b — Codex review of inline work.** When `codex_review: on`, after a task completes inline, dispatch `codex:codex-rescue` (REVIEW mode) with the spec excerpt + diff range `<task_start_sha>..HEAD`. Severity-bucketed findings (high/medium/low). Decision matrix per autonomy mode. Skipped inside Codex with reason `running inside Codex — recursive Codex review disabled`. Skipped when `dispatched_by` indicates the task was Codex-produced (asymmetric rule above).
4. **Step C 4b (wave mode, v5.8.0+) — N parallel Codex reviews per wave.** Pre-v5.8.0 the wave-end Step 4b was skipped entirely on the rationale that wave members don't commit individually. v5.8.0 replaces that skip with N parallel `codex:codex-rescue` REVIEW dispatches at wave end — one per member — using diff range `<wave_start_sha>..<wave_end_sha>` filtered to each member's `**Files:**`. All N dispatches are emitted in a single assistant message per the reviewer-batching trigger. Each review is bounded to its own task's files and produces independently attributed findings. The asymmetric rule applies per member: Codex-produced members are skipped with `decision_source: codex-produced` and the rest get reviewed. Uses `contract_id: codex.review_wave_member_v1`.
5. **Plan annotations override the heuristic.** `**Codex:** ok` forces eligible (delegate even if heuristic rejects); `**Codex:** no` forces ineligible. **v5.8.0 plan-writer default flip:** Step B2 now defaults to `**Codex:** ok` for ALL single-file edits (code AND doc), only emitting `**Codex:** no` when multi-file, ambiguous scope, no known verification, or an explicit scope-out applies. The previous "add ok when obviously well-suited" wording was too conservative — it left mechanical doc/code edits routing to Claude despite `codex.routing: auto` being on. The per-task `**Codex:** no` escape hatch bounds the risk.
...
- **Codex-routing visibility (v5.1.1+, checks #39–#41):** three read-only diagnostics that surface the silent-degradation paths discovered during cosmic-cuddling-dusk investigation. **#39** (`codex_auth_expiry`, repo-scoped): reads `~/.codex/auth.json`, base64url-decodes the JWT `exp` claim from `id_token` and `access_token`, warns when either is expired/expiring or `last_refresh` is > 30 days; skipped silently when codex is not installed. Pairs with #18 (config-vs-plugin mismatch). **#40** (`high_complexity_codex_annotation_gap`, plan-scoped): for `complexity: high` plans, compares `^### Task ` heading count against `**Codex:** (ok|no)` annotation count; INFO-flags zero `**parallel-group:**` annotations. Catches the writing-plans skill silently skipping the high-complexity brief — which suppresses Codex eligibility caching and parallel-wave assembly. **#41** (`missing_codex_degradation_evidence`, plan-scoped): two sub-fires — (a) routing/review forced off with healthy auth and no `codex degraded` event in `events.jsonl` (degrade-loudly visibility violation); (b) routing on with `codex_ping ok` events but zero `routing→[codex]` events (every task ineligible — cross-references #40). All three report-only.
- **Codex review coverage (v5.8.0+, check #43):** per-bundle audit pairing every `wave_task_completed` event in `events.jsonl` with a matching `review→CODEX|SKIP(<reason>)` event carrying explicit `decision_source`. WARNs when coverage < 100% and the run was not inside a Codex host (host-suppressed runs categorically skip review per §8). Backfills against pre-v5.8.0 bundles will flag every wave-mode run because Step 4b was skipped entirely in wave-mode pre-v5.8.0; this is expected and the warning is the design signal. Pairs with the new `wave_codex_review_skip` failure class. Report-only.
- **Codex JWT cosmetic-expiry refinement (v5.2.3+).** Step 3 of CC-2 (boot banner in `commands/masterplan.md`), Check #39 (`codex_auth_expiry`), and Check #41's `auth_healthy` probe now skip sub-conditions (a)/(b) (token-expired and token-expires-within-24h) when `auth_mode == "chatgpt"` AND `tokens.refresh_token` is present AND `last_refresh` is within the last 7 days. ChatGPT mode uses short-lived JWTs that auto-refresh on every codex call via the persistent refresh token; `id_token.exp` being minutes-to-hours past `now` is the normal steady state, not degradation. The (c) sub-condition (`last_refresh > 30d`) still fires under this shape, since that would indicate the refresh token itself has gone stale. All three sites also now read tokens from the nested `.tokens.<field>` JSON path (the schema_v3+ shape of `~/.codex/auth.json`) with a top-level fallback for forward/backward compat. The `codex_health_check_jwt_only` watcher in `lib/masterplan_session_audit.py` — added in v5.2.1 to surface this false-positive class — was retired in v5.2.3 having served its purpose; the user-visible boot banner is the regression detector now (if the false positive returns, it returns visibly on every invocation).
...
### Why Step 0 `scan-then-ping` default (v5.3.0+) and the Doctor #41 `|| echo 0` patch (v5.3.1)

Step 0's `detection_mode` controlled how the orchestrator determined whether the Codex companion plugin was available for routing/review. The original choices (added in v2.0.0) were `ping` (default), `scan`, and `trust`. The `ping` default required the LLM orchestrator to dispatch a 5-token `codex:codex-rescue` Agent and judge the result, then emit a visible-stdout warning if it concluded "not detected." The warning was emitted *before* the `events.jsonl` write that recorded the degradation, so the orchestrator could emit "plugin not detected" without ever proving it had run the ping.

Observed failure: on 2026-05-15, `/loop /masterplan --autonomy=full` in a `.worktrees/pivot-landing-4b-yanos-wireguard` worktree on one host emitted `⚠ Codex plugin not detected — codex_routing and codex_review degraded to off for this run` despite the session's own system-reminder skills list containing `codex:codex-rescue`, `codex:setup`, `codex:rescue`, `codex:gpt-5-4-prompting`, `codex:codex-cli-runtime`, `codex:codex-result-handling`. Independent evidence from `bin/masterplan-codex-usage.sh` (commit b140246) showed the same host actively dispatching Codex in the same window. Forensic deep-dive on the historical `yanos-os/layers/meta-yanos/docs/masterplan/yocto-error-qa-audit` bundle showed the same pattern: 2026-05-09 `codex_degraded — plugin not detected` event, then 2026-05-10 `codex_host_suppressed` retro-fix via `doctor --fix` — confirming the run had actually been executing *inside Codex* (where host-suppression is the correct degrade reason) but Step 0 confabulated "plugin not detected" instead.

v5.3.0 added a 4th `detection_mode` value `scan-then-ping` and made it the default. **Stage A** is a literal substring scan of the system-reminder skills list for `codex:` — zero judgment surface, modeled on the `codex_host_suppressed` precedent at `parts/step-0.md:94` which already trusts a system-reminder substring as authoritative. The `codex:` prefix is structural (Anthropic plugin namespacing), so the signal is robust as long as that namespace convention holds. **Stage B** falls back to the legacy 5-token ping only when Stage A returns zero matches, preserving the "plugin truly absent" detection path. Explicit `detection_mode: ping` in `.masterplan.yaml` keeps ping-only semantics for users who want it; only the unset default flipped.

Initial plan included an "evidence-required guardrail" (require `detection_evidence_*` field in the `codex degraded` event so a deterministic post-hoc check could catch confabulation). Dropped on Plan-agent advice — a model willing to confabulate "ping returned error" will equally confabulate the evidence field. Real load-bearing leverage is post-hoc deterministic detection via Doctor, which is bash-checkable. v5.3.0's contribution to that detection is the `degradation_self_doubt` event: Step 0, when about to emit the degradation warning, checks two on-disk probes (auth healthy + plugin manifest exists on disk) and writes a `degradation_self_doubt` event if both pass. The Doctor #41 sub-fire (c) escalation (ERROR severity) reads this event and surfaces likely confabulation cases.

**v5.3.1 patch** fixed a pre-existing bash bug discovered during the v5.3.0 retroactive lint sweep across 426 bundles. The idiom `grep -cE 'foo' "$events" 2>/dev/null || echo 0` (used at 5 sites in Check #41) produced `"0\n0"` when `$events` was readable with zero matches: `grep -c` always prints `"0"` and exits 1, so `|| echo 0` fired and appended a second `"0"`. The downstream `[ "$var" -eq 0 ]` then errored with `bash: [: 0\n0: integer expected` and silently skipped the if-branch. Net effect since v5.1.1: sub-fire (a) "silent override without evidence" only fired when `events.jsonl` was entirely unreadable, never on the intended "file exists with zero degraded events" case. The fix dropped the `|| echo 0` fallback (grep -c always prints a number when the file is readable), guarded the per-bundle loop with `[ -r "$events" ] || continue`, and used `${var:-0}` parameter expansion in the integer tests. Post-fix sweep surfaced one historical sub-fire (c) ERROR in `yanos-os/yocto-error-qa-audit` (validating the v5.3.0 design) and a sub-fire (a) WARN in `superpowers-masterplan/p4-suppression-smoke` that the bug had been hiding.

Lesson for future Doctor checks: a `grep -c PATTERN file` reads three exit codes (0=matches, 1=zero matches, 2=read error) but always prints stdout. Use `[ -r "$file" ]` guards explicitly and trust grep's stdout — don't paper over with `|| echo N` fallbacks that conflate exit-1 and exit-2.
