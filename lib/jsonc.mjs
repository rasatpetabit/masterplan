 ...
 *
 * Historical production readers of JSONC policy files
 * (the wave dispatcher and the doctor lane-health check) each carried
 ...
 *
 * This is a local implementation rather than an import: the canonical `parseJsonc`
 * lived in an external repo masterplan never imported directly, so a module
 * import would have created a source dependency that neither repo declared.
 */
...
export function stripJsonc(text) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (ch === '/' && next === '*') {
      ...
      // instead of being rejected. Matches the canonical JSONC parser behavior.
 ...
 * string-aware approach the canonical JSONC parser already uses.
 *
 * @param {string} text
 * @returns {string}
 */
