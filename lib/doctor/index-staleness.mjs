      ...
      if (recorded) {
        ...
        if (recorded !== currentHash) {
          findings.push({
            id: ID, severity: 'WARN',
            summary: `bundle ${slug}: plan.md changed since indexing (plan_hash stale) — re-run plan/index`,
            fix: 'no built-in re-index verb exists (mp load-plan stamps plan_hash at load time only); restamp plan.index.json plan_hash from the current plan.md sha256 — a mechanical refresh of the one derived field',
          });
        }
      }
    } catch {
