...
// Absent auth.json -> SKIP (Codex simply isn't installed — not a failure). This is WARN-only and
// purely informational: masterplan's review is cross-vendor (the routing policy's adversary
// panel — no Codex), so codex auth never gates a masterplan run. The check only flags a stale Codex login
// for users who still rely on Codex tooling on this host.
...
export function check(repoRoot, opts = {}) {
  ...
  try {
    ...
    // File exists but is not valid JSON — report as WARN, not SKIP.
    return [{ id: ID, severity: 'WARN', summary: `codex auth.json unreadable: ${err.message}`, fix: FIX }];
  }
