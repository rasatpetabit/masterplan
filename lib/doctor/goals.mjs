  ...
  for (const slug of slugs) {
    try {
      ...
      if (state.status === 'archived') {
        ...
        // Find last goal_check
        const checkEvents = events.filter((e) => e.type === 'goal_check');
        const lastCheck = checkEvents.length > 0 ? evData(checkEvents[checkEvents.length - 1]) : null;
        ...
        if (!validCheck) {
          ...
          const lastWaiver = waiverEvents.length > 0 ? evData(waiverEvents[waiverEvents.length - 1]) : null;
          if (lastWaiver) {
