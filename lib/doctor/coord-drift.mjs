...
import { isTerminalIssueStatus } from '../github-coord.mjs';
...
export function check(repoRoot) {
  ...
  for (const slug of slugs) {
    ...
    for (const [taskIdStr, entry] of Object.entries(issueMap)) {
      ...
      if (localTask?.status === 'done' && !isTerminalIssueStatus(entryStatus)) {
        findings.push({
          id: ID, severity: 'WARN',
          summary: `bundle ${slug}: task ${taskIdStr} is locally done but issue_map status is '${entryStatus ?? 'unset'}' (expected merged/closed)`,
          fix: `run \`mp reconcile-integration\` then record the merge with \`mp update-issue-map --task-id=${taskIdStr} --status=merged --merge-sha=<sha>\` (the issue_map writer — \`mp set-coord\` writes coordination scalars, not per-task entries)`,
        });
      }
