...
export function check(repoRoot, opts = {}) {
  ...
  for (const slug of slugs) {
    ...
    if (state.worktree && !worktrees.has(state.worktree) && !handledPaths.has(state.worktree)) {
      // HOST-LOCAL STATE (the v10.0.2 tag CI found this): the doctor examines ONE repository;
      // a bundle's declared worktree is an absolute path on the host that runs it. A declaration
      // UNDER THIS REPO'S ROOT gets full verification — a path there that is not a registered
      // worktree is the owner-host corruption class (an ERROR, the fixture's case). A declaration
      // under a DIFFERENT root (a CI runner checking out the branch mid-run sees the owning
      // host's absolute path) is another host's state — out of scope here, a SKIP with the
      // reason; the check runs for real on the owning host.
      // PATH-ALIAS-SAFE ownership (the pass-4 review's finding): resolve the declaration
      // before comparing — a RELATIVE declaration resolves against the repo root; a symlink
      // ALIAS of the root (alias -> repo, declared alias/.worktrees/gone) canonicalizes through
      // its deepest existing ancestor, so an owner-host declaration never SKIPS by spelling.
      // Compare against BOTH the lexical repo root and the canonical one (the doctor may run
      // from an aliased cwd; repoGitDirCanonical is the realpath of the git dir).
      // A FOREIGN-PLATFORM absolute declaration (the pass-4 closure re-review's finding: on
      // POSIX a Windows drive path 'C:\\repo\\.worktrees\\gone' or a UNC '\\\\server\\share\\...'
      // is not path.isAbsolute, so it fell into repo-relative resolution and mis-earned an
      // owner-host ERROR). Such a declaration belongs to a machine of another platform — the
      // foreign-host SKIP semantics, before any repo-relative resolution.
      const windowsAbsolute = /^[A-Za-z]:[\\/]/.test(state.worktree) || /^\\\\/.test(state.worktree);
      const declaredAbs = (path.isAbsolute(state.worktree) || windowsAbsolute)
        ? state.worktree
        : (mainRepoRoot ? path.resolve(mainRepoRoot, state.worktree) : state.worktree);
      let canonicalDeclared = declaredAbs;
      try {
        canonicalDeclared = fs.realpathSync.native(declaredAbs);
      } catch {
        // Absent path: canonicalize the deepest EXISTING ancestor and re-append the tail.
        let p = declaredAbs;
        const tail = [];
        ...
        canonicalDeclared = tail.length ? path.join(p, ...tail) : p;
      }
      const canonicalRoot = repoGitDirCanonical ? path.dirname(repoGitDirCanonical) : null;
      const insideThisRepo = !!(mainRepoRoot || canonicalRoot) && (
        (mainRepoRoot && (canonicalDeclared === mainRepoRoot || canonicalDeclared.startsWith(`${mainRepoRoot}${path.sep}`)))
        || (canonicalRoot && (canonicalDeclared === canonicalRoot || canonicalDeclared.startsWith(`${canonicalRoot}${path.sep}`))));
      findings.push(insideThisRepo
        ? {
        ...
        : {
            id: ID, severity: 'SKIP',
            summary: `bundle ${slug}: worktree '${state.worktree}' belongs to another repository root (this doctor examines ${mainRepoRoot}) — host-local state of the bundle's owning host, verified there`,
          });
    }
