...
export function parseConfiguredBackends(policyText) {
  ...
  // Was a whole-line-only comment strip: a trailing `// comment` in the policy made
  // JSON.parse throw and this returned [], reporting the adversary lane as having no
  // backends configured. Use the string-aware parser (lib/jsonc.mjs).
  let policy;
  try { policy = parseJsonc(String(policyText)); } catch { return []; }
