...
export function check(repoRoot, opts = {}) {
  ...
  let slugs;
  try {
    // Skip hidden directories (dot-prefixed). The `.graveyard/` retirement archive lives under
    // docs/masterplan as a sibling of real bundle dirs but is NOT a bundle: its children are the
    // archived bundles, and the archive dir itself intentionally carries no state.yml. Treating it
    // (or any dot-dir) as a slug produced a permanent false-positive WARN on every repo using the
    // archive convention. The other doctor checks that enumerate slug dirs have the same latent
    // gap; this is the one that surfaces it as a finding.
