  ...
  for (const root of roots) {
    ...
    for (const dirName of dirNames) {
      ...
      const record = {
        ...
        topic: state.topic != null ? String(state.topic) : null,
        worktree: state.worktree ?? null,
        goals: readGoalsList(bundleDir, _fs),
        planned_paths: readPlannedPaths(bundleDir, _fs),
        overlap: readOverlapDecision(bundleDir, _fs),
...
// ---- §8 inventory enrichment (run-awareness.core) ----------------------------

function readGoalsList(bundleDir, _fs = fs) {
  try {
    const text = _fs.readFileSync(path.join(bundleDir, 'goals.md'), 'utf8');
    return (parseGoals(text).goals || []).map((g) => ({ id: g.id, text: g.text }));
  } catch {
    return [];
  }
}

// The seed-time overlap decision, reconstructed from the durable overlap_review event
// (the ONLY thing that survives a compaction with the seed's decision). `mp seed` writes it
// second (after bundle_created) with inventory_sha256 + outcome + candidates; this reader
// takes the LAST such event so a re-seed with --force (which replaces the ledger) is
// reflected. Returns null when no overlap_review event exists (pre-review bundles and
// hand-made fixtures), mirroring how goals/planned_paths degrade to empty values rather
// than throwing on absent artifacts.
function readOverlapDecision(bundleDir, _fs = fs) {
  let text;
  try {
    text = _fs.readFileSync(path.join(bundleDir, 'events.jsonl'), 'utf8');
  } catch {
    return null;
  }
  let found = null;
  for (const raw of text.split('\n')) {
    ...
    if (obj && obj.type === 'overlap_review') {
      found = {
        inventory_sha256: typeof obj.inventory_sha256 === 'string' ? obj.inventory_sha256 : null,
...
function readPlannedPaths(bundleDir, _fs = fs) {
  try {
    const idx = JSON.parse(_fs.readFileSync(path.join(bundleDir, 'plan.index.json'), 'utf8'));
    const set = new Set();
    for (const t of idx.tasks || []) for (const f of t.files || []) set.add(String(f));
    return [...set].sort();
  } catch {
    return [];
  }
...
// Canonical serialization of the run inventory the seed-time overlap review is computed over:
// records sorted by slug, each projected to a fixed key set in alphabetical order.
export function serializeInventory(records) {
  const rows = (records || [])
    .map((r) => ({
      goals: r.goals ?? [],
      phase: r.phase ?? null,
      planned_paths: r.planned_paths ?? [],
      slug: r.slug,
...
export function inventorySha256(records) {
  return createHash('sha256').update(serializeInventory(records), 'utf8').digest('hex');
}

// A recorded overlap review binds to the inventory it saw; a changed inventory makes it stale.
export function assertOverlapReviewFresh(review, records) {
  const current = inventorySha256(records);
  if (!review || review.inventory_sha256 !== current) {
    throw new Error(`overlap_review_stale: inventory ${review?.inventory_sha256 ?? 'none'} != ${current}`);
  }
  return true;
}
