...
function requireSkillMd(skillRoot) {
  try {
    ...
    namedFailure('skill_absent', `the installed skill has no readable SKILL.md at ${path.join(skillRoot, 'SKILL.md')}`);
  }
}
...
function loadValidatedSchema(skillRoot, manifest) {
  ...
  try {
    ...
    // A declared core file missing from the tree fails closed as an undeclared dependency
    // (the closed set names it; the tree does not provide it).
    namedFailure('undeclared_dependency', `the declared schema.json is missing or unreadable at ${schemaPath} — a closed-set file must exist in the tree (§5.5)`);
  }
  let schema;
  try {
...
function digestClosedFileSet(set, skillRoot) {
  ...
  for (const rel of [...set].sort()) {
    const bytes = fs.readFileSync(path.join(skillRoot, rel));
    h.update(rel, 'utf8');
    h.update('\n', 'utf8');
