...
/** Resolve a git-printed path against the selected repository. `--git-common-dir` (and
 *  occasionally `--show-toplevel`) can print a path RELATIVE to the `-C` repo; path.resolve()
 *  of a relative string would otherwise follow process.cwd(). */
function resolveGitPath(repo, printed) {
  const p = String(printed ?? '').trim();
  if (!p) throw new Error(`recovery: git printed an empty path in ${repo}`);
  return path.isAbsolute(p) ? path.resolve(p) : path.resolve(repo, p);
}
...
export function canonicalRepoIdentity(repo, _exec = execFileSync) {
  ...
  const top = resolveGitPath(repo, git(['rev-parse', '--show-toplevel']));
  const common = resolveGitPath(repo, git(['rev-parse', '--git-common-dir']));
 ...
 * overrides pin every output-affecting control so ambient *repo-local* gitconfig cannot
 * change the artifact under one format version: core.quotePath, diff.external,
 * diff.algorithm, diff.context, diff.interHunkContext, diff.indentHeuristic,
 * diff.noprefix, diff.mnemonicPrefix, diff.srcPrefix, diff.dstPrefix,
 * diff.orderFile / diff.orderfile (empty order file via /dev/null),
 * diff.suppressBlankEmpty, diff.submodule. Global and system gitconfig are isolated out
 * of the capture process entirely (GIT_CONFIG_GLOBAL=/dev/null, GIT_CONFIG_NOSYSTEM=1);
 * only the selected repo's config remains ambient, and every known output knob in it is
 * pinned. --no-ext-diff and --no-textconv suppress an external driver / textconv;
 * --no-renames, --no-color, --full-index, --binary, --submodule=short keep hunk/index
 * / submodule lines stable. Git's repository-ownership protection is LEFT ON — a
 * capture of a repo the process does not own must fail the way git itself fails; do
 * not pin `safe.directory=*`. Disposable fixtures are created by the same uid as the
 * test process, so ownership checks pass without a trust-all override. If a test ever
 * needs explicit trust of ONE path, inject it via GIT_CONFIG_COUNT on that test's
 * child env — never here.
 * Returns a Buffer of the EXACT captured bytes (no UTF-8 decode, no trimming).
 * `git diff` exits 1 when the delta is nonempty — that is success, not failure.
 */
const CAPTURE_ISOLATED_ENV = {
  GIT_CONFIG_GLOBAL: '/dev/null',
  GIT_CONFIG_NOSYSTEM: '1',
};

/** Lossless JSON carrier for the captured artifact. Buffer's default JSON form is
 *  `{type:'Buffer',data:[...]}` — that MUST never travel as the reviewer's `diff`.
 *  Phase A descriptors carry `diff` as this base64 string + `diff_encoding:'base64'`.
 *  Decode returns the EXACT captured bytes; the digest is always over those bytes. */
export function encodeRecoveryDiff(bytes) {
  const buf = Buffer.isBuffer(bytes) ? bytes : Buffer.from(bytes ?? '');
  return buf.toString('base64');
}

export function decodeRecoveryDiff(encoded) {
  if (typeof encoded !== 'string') {
    throw new Error('recovery: diff carrier must be a base64 string');
  }
...
export function captureCommittedDiff(repo, baseSha, headSha, _exec = execFileSync) {
  const args = [
    ...
    '--full-index', '--binary', '--submodule=short', '-O/dev/null',
    `${baseSha}..${headSha}`,
  ];
  // Env isolation is passed as exec options. Injected `_exec` test doubles that ignore
  // the options object stay compatible; execFileSync consumes `env`.
  const execOpts = {
    encoding: 'buffer',
    stdio: ['ignore', 'pipe', 'pipe'],
    maxBuffer: 64 * 1024 * 1024,
    env: childEnv(CAPTURE_ISOLATED_ENV),
  };
  const run = () => _exec('git', ['-C', repo, ...args], execOpts);
  try {
    const out = run();
    return Buffer.isBuffer(out) ? out : Buffer.from(out ?? '');
  } catch (err) {
    // git diff exits 1 when the two endpoints differ; stdout is the artifact.
    if (err && (err.status === 1 || err.code === 1) && err.stdout != null) {
      return Buffer.isBuffer(err.stdout) ? err.stdout : Buffer.from(err.stdout);
    }
...
/**
 * Frozen-context fingerprint: SHA-256 over the canonical JSON of a *normalized subset*
 * of review_context: {enabled (===true), base_sha (string or null), tasks[] of
 * {task_id, description, class, repo} with nullish fields folded to null}. A non-array
 * tasks value becomes []. Binding consistency between Phase A and Phase B comes from
 * recomputing with this same function, not from covering every review_context field.
 */
