 ...
 * overrides pin every output-affecting control so ambient repo/user gitconfig cannot
 ...
 * diff.noprefix, diff.mnemonicPrefix, diff.srcPrefix, diff.dstPrefix. --no-ext-diff
 * and --no-textconv suppress an external driver / textconv; --no-renames, --no-color,
 * --full-index, --binary keep hunk/index lines stable.
...
export function captureCommittedDiff(repo, baseSha, headSha, _exec = execFileSync) {
  ...
  const run = () => _exec('git', ['-C', repo, ...args], {
