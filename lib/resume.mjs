...
//
// Recovery is derived from disk state alone: a dead run with work outstanding always resumes
// via the legacy `recover_wave` action — reset the wave's incomplete file scope and
// re-dispatch. There is no blackboard-backed replay path: the state field was never written
// by any caller, and the old record-replay action had no handler in the continue loop, so it
// could only surface as a decide-error. Interrupted runs therefore choose only executable
// recovery actions.
//
// Returns one of:
...
//   { action: 'recover_wave', wave, tasks, resetPaths, staleTaskId } — crash mid/pre-wave: reset scope + re-dispatch
...
import { PLANNING_MODES } from './config.mjs';

export function decideNextAction(state = {}, liveness = {}) {
  // The resolved planning mode, when the caller supplies one, outranks the value persisted at
  // seed — config can be changed between the seed and the resume, and the persisted value is
  // then stale. It is validated against the same enum lib/config.mjs enforces: an unknown mode
  // FAILS CLOSED rather than falling back to the stale persisted value, because silently
  // routing a run serial when the operator configured parallel is exactly the substitution the
  // fail-closed rule forbids. Absent (null/undefined) is not invalid — it means "not resolved".
  const resolvedPlanningMode = liveness?.planning_mode === undefined || liveness?.planning_mode === null
    ? null
    : (() => {
      const m = liveness.planning_mode;
      if (typeof m !== 'string' || !PLANNING_MODES.includes(m)) {
        throw new Error(`decideNextAction: planning_mode ${JSON.stringify(m)} is not one of ${PLANNING_MODES.join('/')}`);
      }
      return m;
    })();
  ...
  if (activeRun) {
    // The bootstrap stage owns the bundle while its marker stands: no plan or execute wave
    // may dispatch underneath it, so decide surfaces it rather than routing on it.
    if (activeRun.kind === 'bootstrap') {
      return { action: 'bootstrap_in_flight', marker: activeRun };
    }
  ...
  if (pending.length === 0) {
    ...
    if ((phase === 'brainstorm' || phase === 'plan') && tasks.length === 0) {
      return { action: 'resume_phase', phase, planning_mode: resolvedPlanningMode ?? state?.planning_mode ?? 'auto' };
    }
...
// Recovery for a dead run with work outstanding: reset the wave's incomplete file scope and
// re-dispatch. `staleTaskId` carries the dead run's task_id so the continue loop can reconcile
// the promoted run marker before re-launching.
...
// ---- legacy active_run marker classification (fabric-era reconciliation) --------
//
// classifyLegacyMarker(activeRun) -> null | { legacy, wave?, staleTaskId? }
//
// PURE shape classifier for persisted active_run markers written by the pre-fabric L2
// launch vehicles (the launch_workflow execute/plan kinds). The fabric continue path
// (lib/continue.mjs) consumes this to reconcile stale legacy markers instead of emitting
// probe/reap ops for L2 Workflow handles that no longer exist. Classes use EXACT
// discriminants — a partial/corrupt marker missing any required field classifies
// 'unrecognized', which the consumer surfaces as the unreconcilable ask, NEVER a
// conversion (adversary review P1: no permissive coercion):
//   'plan-launching'    kind:'plan' AND phase:'launching'
//   'plan-promoted'     kind:'plan' AND run_id AND task_id
//   'execute-promoted'  integer wave AND run_id AND task_id
//   'execute-launching' integer wave AND phase:'launching' AND scope/baseline arrays
//   'unrecognized'      any other non-null object — corrupt/unknown; never throws
// Returns null for a null/absent/non-object marker (nothing to reconcile). NOTE: this
// classifies serialized marker DATA — the l2-deletion sweep keeps these shapes as upgrade
// targets, not as live L2 surface.
export function classifyLegacyMarker(activeRun) {
  if (activeRun == null || typeof activeRun !== 'object' || Array.isArray(activeRun)) return null;
  // The bootstrap stage's own marker is its OWN classification. It must never fall through to
  // the plan or execute shapes: converting it would dispatch work underneath the irreversible
  // stage, and calling it 'unrecognized' would invite a manual "recovery" that clears it.
  if (activeRun.kind === 'bootstrap') {
    return { legacy: 'bootstrap', wave: activeRun.wave ?? null };
  }
  if (activeRun.kind === 'plan') {
    if (activeRun.run_id && activeRun.task_id) {
      return { legacy: 'plan-promoted', staleTaskId: activeRun.task_id };
    }
    if (activeRun.phase === 'launching') {
      return { legacy: 'plan-launching' };
    }
    return { legacy: 'unrecognized', wave: activeRun.wave ?? null };
  }
  // A marker that NAMES a kind we do not know is unrecognized, full stop. The shape checks
  // below infer an execute marker from an integer `wave` + run/task ids, and a future stage
  // marker (`bootstrap-v2`, a typo'd `kind`) carries exactly those fields — so falling through
  // would promote it to execute recovery and dispatch a wave underneath a stage that owns the
  // repo. Shape inference is for genuinely old markers, which have no `kind` discriminant at
  // all; anything that declares one must be on the allowlist above.
  if (typeof activeRun.kind === 'string' && activeRun.kind !== '') {
    return { legacy: 'unrecognized', kind: activeRun.kind, wave: activeRun.wave ?? null };
  }
  // execute-promoted: run_id + task_id present. Wave MAY be corrupt (null/non-integer) —
  // the consumer decides reconcilability (not-an-integer → unreconcilable ask). Do NOT
  // require Number.isInteger(wave) here or corrupt promoted markers fall to 'unrecognized'
  // and never hit the consumer's integer check.
  if (activeRun.run_id && activeRun.task_id) {
    return {
      legacy: 'execute-promoted',
      wave: Object.prototype.hasOwnProperty.call(activeRun, 'wave') ? activeRun.wave : null,
      staleTaskId: activeRun.task_id,
    };
  }
  if (
    Number.isInteger(activeRun.wave)
    && activeRun.phase === 'launching'
    && Array.isArray(activeRun.scope)
    && Array.isArray(activeRun.baseline)
  ) {
    return { legacy: 'execute-launching', wave: activeRun.wave };
  }
  return { legacy: 'unrecognized', wave: activeRun.wave ?? null };
}
