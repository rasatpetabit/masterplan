...
//   { action: 'resume_phase', phase }                                 — pre-execute (brainstorm|plan) bundle, no plan built yet: hand to §3's named-phase lifecycle (NOT finalize)
...
export function decideNextAction(state = {}, liveness = {}) {
  ...
  if (pending.length === 0) {
    ...
    if ((phase === 'brainstorm' || phase === 'plan') && tasks.length === 0) {
      return { action: 'resume_phase', phase };
    }
