 ...
 */
export function finishCheckpointEvidence({ statePath, deployBaseSha, chain, liveDigest, skillRoot } = {}) {
 ...
 *
 * §5.5's finish tuple (review round 2 — finding 4): the receipt also names the
 * repository authorization members — repo_intent_digest and target_identity — on the
 * schema-backed family, and they are REQUIRED and COMPARED here against the CURRENT
 * evidence (the fresh reconciliation's authorization digest + the current target
 * identity). Absent or foreign members are the checkpoint's rejection vocabulary
 * (missing / partial / mismatched / stale), never approval; the OBSERVATION half
 * (resolved_commit, resolved_at) never joins the comparison, exactly as the
 * authorization/observation split demands. A legacy bundle reports its absence
 * explicitly. The shared verifier's own tuple checks run FIRST, so the deploy/identity
 * members keep the vocabulary priority for the halves they own.
 */
export function verifyFinishReceipt(receipt, current) {
  ...
  if (!v.ok) return v;
  // The repository authorization members, named outright on the schema-backed family:
  // the receipt carries BOTH and both match the CURRENT evidence. The members are the
  // AUTHORIZATION only — the observation half lives in the evidence beside them and is
  // never compared (§6.3's split).
  if (current.family === 'schema_backed') {
    const gotDigest = receipt?.repo_intent_digest;
    const gotTarget = receipt?.target_identity;
    const digestPresent = gotDigest !== undefined && gotDigest !== null && String(gotDigest) !== '';
    const targetPresent = gotTarget && typeof gotTarget === 'object' && !Array.isArray(gotTarget)
      && ['repository', 'remote', 'ref'].every(
        (k) => typeof gotTarget[k] === 'string' && gotTarget[k].trim() !== '',
      );
    if (!digestPresent && !targetPresent) {
      return {
        ok: false,
        status: 'missing',
        reason: 'the receipt names neither repository authorization member — a final assessment binds repo_intent_digest and target_identity outright (§5.5/§6.3), and a receipt bound to nothing is the unearned completion §7.1 exists to prevent',
      };
    }
    if (!digestPresent) {
      return {
        ok: false,
        status: 'partial',
        reason: 'the receipt carries no repo_intent_digest — the repository authorization half of the finish tuple is partial, and a member that should exist is absent (§5.5)',
      };
    }
    if (!targetPresent) {
      return {
        ok: false,
        status: 'partial',
        reason: 'the receipt carries no target_identity — the repository authorization half of the finish tuple is partial, and a member that should exist is absent (§5.5)',
      };
    }
    if (String(gotDigest) !== current.repo_intent_digest) {
      return {
        ok: false,
        status: 'stale',
        reason: `the receipt's repo_intent_digest (${String(gotDigest).slice(0, 19)}…) differs from the current one (${String(current.repo_intent_digest).slice(0, 19)}…) — it is a receipt about a different repository intent, and the authorization is the whole comparison (§6.3)`,
      };
    }
    const wantTarget = current.target_identity;
    const differingTarget = ['repository', 'remote', 'ref'].filter((k) => gotTarget[k] !== wantTarget[k]);
    if (differingTarget.length > 0) {
      return {
        ok: false,
        status: 'mismatched',
        reason: `the receipt's target_identity differs from the current one on ${differingTarget.join(', ')} — it is a receipt about a different integration target, and no receipt crosses that authorization boundary (§6.3)`,
      };
    }
  }
  return {
...
// ---- the finish checkpoint bound to the recorder/replay surfaces (task 56, the
// production consumers) --------------------------------------------------------
//
// finishCheckpointIdentity is the FINISH checkpoint's evidence composed WITHOUT the
// deploy members — for the two surfaces that must judge a receipt's identity binding
// but do not own the deploy tuple: `mp record-goal-check --final` (which supplies the
// deploy members itself, from its own flags, and must refuse a receipt whose
// intent_identity is foreign or absent) and the replay/receipt surfaces. Composing from
// the same producers keeps ONE finish identity everywhere: the identity half of
// finishCheckpointEvidence, identical values, derived from the bundle alone.
export function finishCheckpointIdentity({ statePath, skillRoot } = {}) {
  if (!statePath) throw new Error('finishCheckpointIdentity: statePath is required');
  const identity = buildIntentIdentity({ statePath, skillRoot });
  if (!identity.ok) return identity;
