...
export function finishStep({
  ...
  if (state.pending_gate) {
    ...
    if (gate === 'branch_finish') {
      ...
      const reviewRecord = head ? selectReviewAtHead(events, head) : { present: false };
      Object.assign(out, {
        head,
        branch,
        base,
  ...
  if (goalsEnabledFor(state, events)) {
    ...
    if (specHash && !selectReentry(events, { kind: 'artifact-hash', gate: 'spec', key: specHash }).present) {
      return {
  ...
  if (
    ...
    !selectReviewAtHead(events, head).present && !hasCodexSkipAtSha(events, head)
  ) {
  ...
  writeState(absState, state);
  const reviewRecord = selectReviewAtHead(events, head);
