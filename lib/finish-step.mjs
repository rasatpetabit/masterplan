...
export function finishStep({
  ...
  // 7. Whole-branch codex review — durable re-entry guard keyed on the WT code tip.
  //
  // 7a. Effective review setting (spec §4.2-C defensive arm). Legacy bundles seeded before
  //     codex.review became default-on have no state.codex block; codexArmed(undefined) is false,
  //     so the gate would silently fall through. Defensively arm those once — emit a durable
  //     event so the audit trail shows the legacy state was rescued (NOT a silent re-arm). The
  //     event is presence-scoped (not sha-scoped): a defensive arm is a one-time per-bundle fact.
  const stateCodexArmed = codexArmed(state.codex?.review);
  const defensiveArmed = state.codex === undefined; // legacy bundle: no state.codex at all
  const effectiveArmed = stateCodexArmed || defensiveArmed;
  if (defensiveArmed && !hasEventType(events, 'codex_review_defensively_armed')) {
    appendEvent(absState, {
      type: 'codex_review_defensively_armed',
      ts,
      summary: 'codex review defensively armed — legacy bundle missing state.codex.review; finish-step gate would otherwise silently skip',
      data: { sha: head },
    });
  }
  ...
  //     events.jsonl; the existing \bcodex\s+review\b audit regex catches the SUCCESS-summary
  //     format ("codex review complete ...") but the skip summary uses a hyphenated phrase
  //     ("codex review skipped ...") so the proposed-but-not-implemented
  //     codex_review_configured_but_zero_invocations audit can still fire when configured + skipped.
  const skipReason = !effectiveArmed ? 'state.codex.review not armed'
                   : codexSuppressed ? 'codex_host_suppressed'
                   : !base ? 'no_base_branch'
                   : null;
  if (skipReason && !hasCodexSkipAtSha(events, head)) {
    appendEvent(absState, {
      type: 'codex_review_skipped',
      ts,
      summary: `codex review skipped — ${skipReason}`,
  ...
  if (
    effectiveArmed && base && !codexSuppressed && codex === null &&
    !selectCodexReviewForHead(events, head).present && !hasCodexSkipAtSha(events, head)
  ) {
    return { op: 'run_codex_review', base, head, wt: WT, digest_path: digestPath, base_source: baseSource };
  }
  ...
  // Spec §4.2-D: surface suppression / skip reason in the AUQ payload so the user can see WHY
  // review didn't run, not just that it didn't. Skipped at SHA → typed reason from the event;
  // suppressed → the host-detect signal; defensive-armed → the audit-trail note. Re-read events
  // here (NOT reuse the in-memory `events` from step 7): the just-emitted skip event must be
  // visible to lastSkip, and events was read before the appendEvent above.
  ...
  const lastSkip = (() => {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
      ...
      if (rec?.type === 'codex_review_skipped' && rec.data?.sha === head) return rec.data.reason ?? null;
    }
    return null;
  })();
  const codexNotice = codexSuppressed ? 'codex review skipped — codex_host_suppressed (Codex is hosting the command)'
                     : lastSkip ? `codex review skipped — ${lastSkip}`
                     : defensiveArmed ? 'codex review defensively armed (legacy bundle missing state.codex.review)'
                     : undefined;
  return {
    op: 'ask',
    ask: 'gate',
