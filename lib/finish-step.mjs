...
import { orderDeployGroups, classifyStepOutcome, substituteVersionSafe, deployChainHash, liveCheckDigest, versionAtRevision, tagExists, mergeIdentity, mergedTree, auditDeployBoundary, dirtyOutsideBundle } from './finish.mjs';
...
function applyShellAnswers(ctx) {
  ...
  if (ctx.deployStepDone) {
    ...
    if (group === 'user_only') {
      ...
      if (outcome === 'done') {
        appendEvent(ctx.absState, { type: 'deploy_step', ts: ctx.ts, group, index, exit, status: 'done', ...(checkExit === null ? {} : { check_exit: checkExit }), ...(digest ? { digest, digest_path: evidenceRel } : {}), sha: deployBaseSha, ...produced });
...
function finalReceiptFor(ctx, { deployBaseSha, chainHash, liveDigest }) {
  ...
  // has since been amended is stale however well-formed it looks.
  const expectedGoalsHash = goalsMd ? goalsHash(goalsMd) : null;
