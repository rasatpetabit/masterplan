...
//   • { op:'run_adversary_review', base, head, wt, digest_path, fallback_reviewers, fallback_reason? }
...
//                                                   fallback_reviewers (review-fallback) is the
//                                                   ordered fallback list resolved HERE in lib —
//                                                   the adversary class chain, primary excluded,
//                                                   or the config override; the shell dispatches
//                                                   mp-fallback-reviewer under the adversary class
//                                                   per entry, in order, and
//                                                   never re-derives routing policy itself. An
//                                                   empty list carries fallback_reason (why).
...
import { resolveDoneAtRevision, doneDigest, resolveRunConfig } from './config.mjs';
import { selectReentry, reentryEventTypes } from './reentry-guard.mjs';
import { adversaryFallbackReviewers } from './dispatch/routing-policy.mjs';
...
function selectReviewAtHead(eventsText, sha) {
  ...
  const { reviewer, fallback } = reviewDetailAtHead(eventsText, sha);
  return { present, digest, count, base, reviewer, fallback };
}

// review-fallback: the reviewer identity + fallback provenance of the whole-branch review
// recorded at a HEAD sha, projected for the branch_finish AUQ payload (spec §4.2-D — the
// review line shows WHO reviewed, and says so + why when it was a fallback). Read through
// the reentry-guard's own head-sha done-event vocabulary (never a copied type list), beside
// — not inside — selectReentry, whose exact projection shape is pinned by its own tests.
function reviewDetailAtHead(eventsText, sha) {
  const { done } = reentryEventTypes('head-sha');
  let hit = null;
  for (const line of String(eventsText ?? '').split('\n')) {
    ...
    if (!rec || !done.includes(rec.type) || !rec.data || rec.data.sha !== sha) continue;
    hit = rec;
  }
  if (!hit) return { reviewer: null, fallback: null };
  const d = hit.data;
  return {
    reviewer: typeof d.reviewer === 'string' && d.reviewer !== '' ? d.reviewer : null,
    fallback: d.fallback && typeof d.fallback === 'object' && !Array.isArray(d.fallback)
      ? structuredClone(d.fallback)
      : null,
  };
}

// review-fallback: the finish-gate fallback reviewer list, resolved deterministically HERE
// (lib) and carried in the run_adversary_review op payload so the shell never re-derives
// routing policy. Precedence follows the config chain (repo .masterplan.yaml > user > default):
//   - `adversary_review_fallback: off`  → no fallback ([]); a failed primary skips as before
//   - `adversary_review_fallback: [...]` → that list REPLACES the policy-derived default
//   - unset (default)                   → the adversary class chain, primary excluded,
//                                          de-duplicated (routing policy)
// A routing-policy outage is NOT a finish failure (fail-soft, never wedge the gate): the list
// comes back empty, the reason is recorded in the payload, and the gate behaves as it did
// before the fallback existed. A malformed config still throws — that is an operator error
// the other config-reading verbs already fail on, never a silently-dropped control.
//
// A CONFIGURED entry outside the adversary class chain is the same kind of operator error and
// fails closed the same way: the fallback reviewer is dispatched under the adversary class
// (task class `adversary`) and Pi's spawn guard authorizes an explicit model override only
// when it sits inside that class's chain, so such an entry could never run — dropping it
// silently would discard a reviewer the operator asked for. A policy OUTAGE is different: the
// chain is unknown, so the configured list passes through unchanged and a model the harness
// cannot authorize stays the documented per-entry failed attempt.
function finishReviewFallback(repoRoot) {
  const configured = resolveRunConfig({ cli: {}, repoRoot }).values.adversary_review_fallback;
  if (configured === 'off') {
    return {
      fallback_reviewers: [],
      fallback_reason: 'adversary_review_fallback: off — no fallback reviewer configured; a failed primary review skips as before the fallback existed',
    };
  }
  const { reviewers, chain, reason, primary } = adversaryFallbackReviewers();
  if (Array.isArray(configured)) {
    if (primary !== null) {
      const authorized = new Set(chain);
      const outside = configured.filter((m) => !authorized.has(m));
      if (outside.length > 0) {
        throw new Error(
          `adversary_review_fallback names ${outside.map((m) => JSON.stringify(m)).join(', ')} — outside the `
          + `routing policy's adversary class chain (${chain.join(', ') || 'empty'}). The fallback reviewer is `
          + 'dispatched under the adversary class, whose spawn guard authorizes a model override only inside that '
          + "chain, so the entry could never run; fix the entry or use 'off'.",
        );
      }
    }
    return { fallback_reviewers: [...configured] };
  }
  return reviewers.length > 0
    ? { fallback_reviewers: reviewers }
    : { fallback_reviewers: [], fallback_reason: reason };
}
...
function applyShellAnswers(ctx) {
  ...
  if (ctx.review === 'done') {
    ...
    // The machine-readable verdict (the panel's finding: a blocking review was prose-only, so
    // the bootstrap's corrective-trigger machinery could not read it). Validated enum, optional:
    // absent = no verdict field (back-compat with every existing caller).
    if (ctx.reviewVerdict) record.data.verdict = ctx.reviewVerdict;
    // review-fallback: the reviewer identity + (when a fallback ran) why the primary was
    // replaced. Both optional — an old caller that records neither is unchanged, and the
    // branch_finish AUQ payload rehydrates them from here via reviewDetailAtHead.
    if (ctx.reviewReviewer) record.data.reviewer = ctx.reviewReviewer;
    if (ctx.reviewFallbackReason) record.data.fallback = { reason: ctx.reviewFallbackReason };
...
function evaluateFinishMachine(ctx) {
  ...
  if (
    ...
    return {
      op: 'run_adversary_review', base, head: ctx.head, wt: ctx.WT, digest_path: ctx.digestPath, base_source: baseSource,
      ...finishReviewFallback(ctx.MAIN),
    };
  }
...
export function finishStep({
  ...
  reviewReason = null,
  reviewVerdict = null, // 'approve' | 'revise' | 'rework' | 'reject' — the review's machine-readable verdict (absent = no verdict field)
  reviewReviewer = null, // review-fallback: the reviewer that produced the review (model ref or class) — recorded on the durable event's data.reviewer
  reviewFallbackReason = null, // review-fallback: set when the recorded review came from a FALLBACK reviewer — why the primary was replaced
  ...
  // The machine-readable review verdict is a validated enum — an arbitrary string must never
  // land on the durable record the bootstrap's corrective-trigger machinery reads.
  const REVIEW_VERDICTS = ['approve', 'revise', 'rework', 'reject'];
  if (reviewVerdict !== null && !REVIEW_VERDICTS.includes(reviewVerdict)) {
    throw new Error(`finish-step: --review-verdict must be one of ${REVIEW_VERDICTS.join('|')}, got ${JSON.stringify(reviewVerdict)}`);
  }
  // review-fallback: the reviewer identity + fallback reason land on the DURABLE event, so a
  // garbage value must never reach it. A fallback reason describes the review that WAS run —
  // it is only valid alongside --review-done (with a skip, or with nothing, it is a caller bug:
  // dropping it silently would record a fallback review with no trace of why it happened).
  if (reviewFallbackReason !== null && review !== 'done') {
    throw new Error('finish-step: --review-fallback-reason is only valid with --review-done — it names why the primary reviewer was replaced by the fallback that produced this review');
  }
  for (const [flagName, v] of [['--review-reviewer', reviewReviewer], ['--review-fallback-reason', reviewFallbackReason]]) {
    if (v !== null && (typeof v !== 'string' || v.trim() === '')) {
      throw new Error(`finish-step: ${flagName} must be a non-empty string, got ${JSON.stringify(v)}`);
    }
  }
  const ctx = {
    ...
    verify, review, reviewCount, reviewBase, reviewDigestFile, reviewReason, reviewVerdict,
    reviewReviewer, reviewFallbackReason,
