...
import { orderDeployGroups, classifyStepOutcome, substituteVersionSafe, deployChainHash } from './finish.mjs';
...
function applyShellAnswers(ctx) {
  ...
  if (ctx.deploySkip) {
    ...
    appendEvent(ctx.absState, { type: 'incomplete_authorized', ts: ctx.ts, reason: `deploy_skip:${group}[${index}]` });
  }
  if (ctx.deployAttest) {
    ...
    appendEvent(ctx.absState, { type: 'deploy_step', ts: ctx.ts, group, index, exit: 0, status: 'attested', sha: base.sha });
  }
  if (ctx.deployAbort) {
    ...
    appendEvent(ctx.absState, { type: 'incomplete_authorized', ts: ctx.ts, reason: 'deploy_abort' });
  }
  ...
  if (ctx.deployStepDone) {
    ...
    if (group === 'user_only') {
      ...
      if (checkExit === 0) {
        appendEvent(ctx.absState, { type: 'deploy_step', ts: ctx.ts, group, index, exit: 0, status: 'done', check_exit: 0, sha: deployBaseSha });
      } else if (checkExit === 1) {
        appendEvent(ctx.absState, { type: 'deploy_step_check_failed', ts: ctx.ts, group, index, sha: deployBaseSha });
        return { op: 'ask', ask: 'handback', group, index, text: step.text, check: step.check };
      } else {
        appendEvent(ctx.absState, { type: 'deploy_indeterminate', ts: ctx.ts, group, index, exit: 0, check_exit: checkExit, sha: deployBaseSha });
      ...
      if (outcome === 'done') {
        appendEvent(ctx.absState, { type: 'deploy_step', ts: ctx.ts, group, index, exit, status: 'done', ...(checkExit === null ? {} : { check_exit: checkExit }), ...(digest ? { digest } : {}), sha: deployBaseSha });
      } else if (outcome === 'failed') {
        appendEvent(ctx.absState, { type: 'deploy_failed', ts: ctx.ts, group, index, exit, ...(exit === 0 && checkExit === 1 ? { check_exit: 1 } : {}), sha: deployBaseSha });
        return { op: 'ask', ask: 'gate', gate: 'deploy_failed', group, index, error: 'step failed', choices: ['retry', 'skip', 'abort'] };
      } else {
        appendEvent(ctx.absState, { type: 'deploy_indeterminate', ts: ctx.ts, group, index, exit, check_exit: checkExit, sha: deployBaseSha });
        return { op: 'ask', ask: 'gate', gate: 'deploy_indeterminate', group, index, choices: indeterminateChoices(step) };
      }
    }
  }
...
function evaluateFinishMachine(ctx) {
  ...
  if (disp === 'removed_after_merge' || prMerged || hasDeployBase) {
...
function evaluateDeployStage(ctx) {
  ...
  if (events.some((e) => e.type === 'incomplete_authorized' && (e.reason === 'no_definition_of_done' || e.reason === 'deploy_abort'))) return null; // stage resolved: the run archives incomplete
  ...
  if (!deployBaseEvent) {
    ...
    let branchTip = sha;
    try {
      const parents = runGit(ctx.MAIN, ['rev-list', '--parents', '-n', '1', sha]).trim().split(/\s+/);
      if (parents.length >= 3) branchTip = parents[2];
    } catch { /* not a merge */ }
  ...
  for (const group of groups) {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      if (outcome === 'failed' || outcome === 'indeterminate') {
        ...
        return { op: 'ask', ask: 'gate', gate, group, index, choices: outcome === 'failed' ? ['retry', 'skip', 'abort'] : indeterminateChoices(step) };
      }
      const authorizedEvent = events.find((e) => e.type === 'deploy_step_authorized' && e.group === group && e.index === index && e.sha === deployBaseSha);
      // Every authorization gets its own started transition: look for a start AFTER the latest authorization.
      let lastAuthIdx = -1;
      events.forEach((e, k) => { if (e.type === 'deploy_step_authorized' && e.group === group && e.index === index && e.sha === deployBaseSha) lastAuthIdx = k; });
      const startedEvent = events.slice(lastAuthIdx + 1).find((e) => e.type === 'deploy_step_started' && e.group === group && e.index === index && e.sha === deployBaseSha);
      if (group === 'user_only') {
        return { op: 'ask', ask: 'handback', group, index, text: step.text, check: step.check };
      }
      if (!authorizedEvent) {
      ...
      if (startedEvent) {
        ...
        if (step.check && probe === 0) {
          appendEvent(ctx.absState, { type: 'deploy_step', ts: ctx.ts, group, index, exit: 0, status: 'done', check_exit: 0, sha: deployBaseSha, source: 'recovery-probe' });
          continue;
        }
        if (step.check && probe === 1) {
          appendEvent(ctx.absState, { type: 'deploy_failed', ts: ctx.ts, group, index, exit: 0, check_exit: 1, sha: deployBaseSha });
          return { op: 'ask', ask: 'gate', gate: 'deploy_failed', group, index, error: 'started step absent on recovery probe', choices: ['retry', 'skip', 'abort'] };
        }
        appendEvent(ctx.absState, { type: 'deploy_indeterminate', ts: ctx.ts, group, index, exit: 0, check_exit: step.check ? probe : -1, sha: deployBaseSha });
        return { op: 'ask', ask: 'gate', gate: 'deploy_indeterminate', group, index, choices: indeterminateChoices(step) };
      }
...
export function finishStep({
  ...
  const ctx = {
    ...
    deployRetry, deployRerun, deploySkip, deployAttest, deployAbort, merged, mergeSha,
  };
