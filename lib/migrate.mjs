...
// collapses to a one-shot 5.x -> 6.0 migrate; 6.x passes through; pre-5.0 is REFUSED loudly (R3:
// a silently-broken upgrade re-opens a defect the owner already paid for). Evidence: a real
// in-flight 5.0 bundle with a mixed-status task list exists (petabit-datasheets/wbn-datasheet-
// redesign), so the task-list extractor is real code, not a dead path.
//
...
const GUIDANCE =
  'Finish this run under masterplan v7, or re-import via `Use masterplan brainstorm`. ' +
  'The original state.yml is preserved.';
...
// Tolerant across quote styles ('5.1' | "5.0" | 5.0) and a leading `---` doc marker.
...
// v8 cares only done-vs-not (lib/resume.mjs filters `status !== 'done'`). An in_progress task from
...
//   5.x     -> targeted extract + one-shot field map to 6.0.
...
export function migrate(text = '') {
  ...
  if (major >= 6) return parseState(text);
  if (major === 5) return mapLegacyToV8(extractLegacyFields(text));
