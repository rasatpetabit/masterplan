...
// v9→v10 compatibility rule (spec §4.3): pre-v10 bundles may carry complexity_source,
// predecessor_transcript, nullable historical values, or autonomy:'full'. These fields are
// read-only pass-throughs — never validated, normalised, or dropped. effectiveAutonomy()
// derives the in-memory view without writing back.
...
// v9→v10 compatibility rule (spec §4.3): effectiveAutonomy is the derived in-memory view of a
// bundle's autonomy knob. 'full' is treated as 'loose' (the v10 semantics); 'gated' and
// null/undefined map to 'gated'. This is never written back to state.
export function effectiveAutonomy(state) {
  const a = state && state.autonomy;
  if (a === 'full' || a === 'loose') return 'loose';
  return 'gated'; // covers 'gated', null, undefined, and any other value
}
