...
// 2026-07-15 false-positive fix: detectHost was a pure OR of three signals, so
// agentsMdPresent ALONE flipped isCodex:true. On this host every repo carries an
// AGENTS.md, so every Claude Code session pattern-matched as a Codex host (the §0
// skill text passes --agents-md whenever an AGENTS.md is present, making the
// misdetection systematic); only a manual operator override kept simplify-dedup-2
// on the parallel Workflow path. agents-md is therefore a corroborating signal
// only: isCodex requires agentIsCodex OR codexNativeTools. reasons[] still lists
// every present signal for observability.
...
//   agentsMdPresent  — an AGENTS.md compatibility map is present (corroborating only — never sufficient alone)
export function detectHost(signals = {}) {
  ...
  return { isCodex: Boolean(signals.agentIsCodex || signals.codexNativeTools), reasons };
}
