...
// resolveTaskBackend is exercised from prepareWave when config.implementer.qctl.enabled === true
// (C6 deleted the pre-resolved target/backend payload; prepareWave's class-only fabric payload is
// the only dispatch path) — a qctl-eligible task's payload gains the `backend` work-item
// discriminator that lib/dispatch/dispatch-digest.mjs carries onto the dispatch descriptor
// (descriptor-only, never hashed). The seam therefore stays reachable and
// tested (test/qctl-fabric-seam.test.mjs).
