...
//
// CONSUMERS (the dormant-seam reachability contract, docs/design/qctl-multi-repo-apply.md):
// besides the legacy routing-brain path in lib/wave.mjs (slated for post-soak deletion),
// resolveTaskBackend is exercised from prepareWave's FABRIC branch when
// config.implementer.qctl.enabled === true — a qctl-eligible task's payload gains the
// `backend` work-item discriminator that lib/dispatch/dispatch-digest.mjs carries onto the
// adsp descriptor (descriptor-only, never hashed). The seam therefore stays reachable and
// tested (test/qctl-fabric-seam.test.mjs) after the legacy path is deleted.
