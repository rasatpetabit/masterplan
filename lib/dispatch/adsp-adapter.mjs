...
//
// STATUS (adsp-v1): BUILT + UNIT-TESTED BUT NOT YET WIRED into the execution path.
// dispatchTask / escalateCrossReview / revertCrossReview have NO production callers today
// (only test/adsp-adapter.test.mjs). The live wave-execution path is mp-implementer inline
// (workflows/execute.workflow.js); routeTask's `target` is informational-only (see
// docs/conventions/plan-annotations.md — the "future implementer tier" this adapter is
// staged to become). escalateCrossReview resolves the masterplan.mjs bin via
// `resolveMasterplanBin` (lib/paths.mjs) — portable across accounts, no hardcoded /home/<user>.
//
