...
/**
 * Translate ONE broker dispatch_task-shaped result into an mp-implementer digest.
...
export function translateBrokerResult(taskId, brokerResult) {
  ...
  if (digest === null) {
    ...
    // stateless edit tool (skynet_edit_files), not an mp-implementer agent —
  ...
  // Translate to the exact mp-implementer shape (task_id from canonical input),
...
export async function dispatchTask(task, options = {}) {
  ...
  // 5-8. Translate the broker result into the exact mp-implementer digest shape
