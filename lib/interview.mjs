...
function copySchemaSnapshotBytes({ skillRoot, statePath, expectedSchemaSha256 }) {
  ...
  fs.writeFileSync(snapshotPath, schemaBytes);
  const written = fs.readFileSync(snapshotPath);
...
export function readSchemaSnapshot({ statePath } = {}) {
  ...
  try {
    ...
    throw new Error(
      `schema_snapshot_missing: the bundle snapshot ${snapshotPath} is missing while a schema_captured event exists — never fall back to the live skill file (§5.5); re-capture is impossible on a recorded bundle, so the snapshot must be restored from the bundle's own history`
    );
  }
