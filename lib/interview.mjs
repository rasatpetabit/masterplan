...
export function endInterview({ statePath, reason, criticUnavailableAck }) {
  ...
  if (reason === 'converged') {
    if (!floorMet) throw new Error('converged requires the answer floor met');
    if (!intentFloorMet) throw new Error('converged requires the intent floor met');
    if (!roundsMet) throw new Error('converged requires the intent-round minimum met');
    if (!hasDraft || !draftIsLatest) {
      throw new Error('converged requires an interview_draft as the latest intent-content event');
    }
    ...
    if (!floorMet) throw new Error('critic_off requires the answer floor met');
    if (!intentFloorMet) throw new Error('critic_off requires the intent floor met');
    if (!roundsMet) throw new Error('critic_off requires the intent-round minimum met');
  }
...
export function waiveInterview({ statePath, reason }) {
  const r = replayInterview(statePath);
  assertNotTerminal(r);
  appendEvent(statePath, { type: 'interview_waived', reason });
}
...
export function replayLedger(statePath) {
  ...
  for (const ev of r.events) {
    if (ev.type === 'interview_question') {
      ...
      ledger.push({ type: 'end', reason: ev.reason });
