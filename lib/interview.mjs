...
export function endInterview({ statePath, reason, criticUnavailableAck, coverageFile, probingMinimum }) {
  const r = replayInterview(statePath);
  assertNotTerminal(r);
...
export function waiveInterview({ statePath, reason, probingMinimum }) {
  const r = replayInterview(statePath);
  assertNotTerminal(r);
  ...
  if (policy === 'legacy') {
    ...
    const probing = evaluateProbing({ statePath, r, probingMinimum });
    if (r.asked >= r.budget.cap && !probing.met) cause = 'probing_minimum_unmet_at_cap';
  }
  appendEvent(statePath, { type: 'interview_waived', reason, policy, ...(cause ? { cause } : {}) });
}
