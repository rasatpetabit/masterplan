...
export function captureSchema({ statePath, skillRoot } = {}) {
  ...
  const event = {
    ...
    // The absolute skill root THIS capture resolved (review finding 1): the runtime guard's
    // default resolution reads it from the ledger, never from the developer checkout — the
    // identity recorded here is the identity of the skill at THIS root, so the guard that
    // recomputes "the installed skill" must recompute the same root. The path is resolved
    // absolute (an alias or a relative flag would record a root that later resolves elsewhere).
