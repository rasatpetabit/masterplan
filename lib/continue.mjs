...
//   • { op:'launch_workflow', workflow:'execute', cwd, args?, next? }
  ...
  for (let step = 0; step < MAX_DECIDE_STEPS; step++) {
    ...
    switch (action.action) {
      ...
      case 'recover_plan_run': {
        ...
        return buildPlanFanoutOp({ cwd: MAIN, specPath: resolveSpecPath(state, bundleDir) });
      }
...
function dispatchWave(state, absState, planIndexPath, wave, wt, opts) {
  ...
  const fabric = opts.fabricDispatch === true || state.dispatch?.fabric === true;
...
export async function dispatchPlanFanout({
  ...
  try {
    ...
    const fanout = await client.callTool('dispatch_fanout', { descriptors, fail_mode: 'isolated' });
    const arr = Array.isArray(fanout?.results) ? fanout.results : null;
    if (arr === null) {
      // Planning is fully re-derivable — a whole-fan-out failure is a loud stop,
      // never an empty fragment set the merge would happily consume.
      const reason = typeof fanout?.error === 'string' ? fanout.error : 'dispatch_fanout returned no results array';
      throw new Error(`dispatch-plan: broker fan-out failed: ${reason}`);
    }
    results = arr;
  } finally {
    if (!usingInjected) {
