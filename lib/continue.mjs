...
function dispatchWave(state, absState, planIndexPath, wave, wt, opts) {
  ...
  if (opts.codexSuppressed) {
    // Residual 3B — the Codex full-lifecycle path: a suppressed host has no Workflow tool, so the
    // wave dispatches FOREGROUND-SEQUENTIAL in the host's own session (Codex inline; a CC fallback
    // runs one Agent per task, sequentially). Same routed payloads, same frozen scope/baseline,
    // same phase-1 launching marker — so a crash mid-wave resumes through the identical
    // recover_and_redispatch path (no task_id is ever promoted; there is no background task to
    // probe). The host assembles the standard result shape ({wave, tasks:[{task_id, digest}]})
    // and feeds it to the SAME `mp record-result` transaction.
    return {
