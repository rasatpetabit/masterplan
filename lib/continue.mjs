...
// ONE read-only planning work item (dispatch_task descriptor). It carries no write-scope
// fields (files/worktree) — `read_only: true` IS the capability declaration, and the
...
//
// `repo` is REQUIRED even though this lane never writes. It was omitted until 2026-08-07 on
// the premise that a read-only lane must carry no repo/files at all; that was true of
// `dispatch_fanout`, which has since been retired (agent-dispatch mcp.mjs -32601,
// "use harness-native fanout", G2/native-fanout cutover). The surviving route is
// `dispatch_task`, whose normalizeDescriptor() rejects any descriptor without a non-empty
// `repo` — read-only or not — so every planner work item failed to dispatch with
// "Task descriptor requires a non-empty repo" and parallel planning could not run at all.
// `repo` there is a locus/identity field, not a write grant; write scope is gated on
// `read_only` (see the write-scoped-descriptor lease check in mcp.mjs).
export function buildPlanWorkItem(s, { roots, specPath, repoRoot }) {
  ...
  return {
    ...
    read_only: true,
    repo: repoRoot, // broker-required locus (normalizeDescriptor); NOT a write grant
    roots,
    subsystem: s.key,
    brief,
