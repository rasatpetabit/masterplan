 ...
 *
 * §6.3 (review round 2 — finding 5): the record is VALIDATED at consumption, not just
 * parsed. A durable record that no longer satisfies what recordReconciliation persisted —
 * a rows array gutted to [], a row missing its verdict or its section digest, a verdict
 * outside the frozen snapshot's reconciliation vocabulary, a collapsed/foreign status —
 * is the unreadable/partial vocabulary, never a pass. The frozen snapshot's
 * reconciliation_verdicts is the vocabulary AUTHORITY (when a snapshot is readable on
 * this bundle), exactly as buildReconciliationRows validated against when the record was
 * earned.
 */
export function readRecordedReconciliation(statePath) {
  ...
  // A recorded reconciliation is one of the two recordable §6.3 states; unknown_unavailable
  // is NEVER recorded (recordReconciliation refuses it), so a record carrying it was not
  // written by the recorder — treat it as the unreadable record it is.
  const status = parsed.status;
  if (status !== 'resolved' && status !== 'absent_source') {
    throw new Error(
      `readRecordedReconciliation: ${RECONCILIATION_FILENAME} carries status ${JSON.stringify(status)} — not one of the recorded §6.3 states (resolved, absent_source); an unresolved target is a named failure that was never recordable, so the record is unreadable as an authorization — re-earn the reconciliation`
    );
  }
  // The authorization half must be complete: repository/remote/ref name the target and
  // the artifact digest is either a sha256 or the explicit verified absence.
  const id = parsed.identity;
  for (const k of ['repository', 'remote', 'ref']) {
    if (typeof id[k] !== 'string' || id[k].trim() === '') {
      throw new Error(`readRecordedReconciliation: ${RECONCILIATION_FILENAME} is malformed (identity.${k} is not a non-empty string) — re-earn the reconciliation`);
    }
  }
  const digest = parsed.artifact_digest;
  if (digest !== REPO_INTENT_DIGEST_ABSENT && !/^sha256:[0-9a-f]{64}$/.test(String(digest))) {
    throw new Error(
      `readRecordedReconciliation: ${RECONCILIATION_FILENAME} carries artifact_digest ${JSON.stringify(digest)} — neither a sha256 digest nor the explicit verified absence; the record is unreadable as an authorization — re-earn the reconciliation`
    );
  }
  // An absent-source target HAS no rows by construction (nothing to reconcile against);
  // a resolved target REQUIRES its per-section rows, each complete. Rows against an
  // absent source, or none on a resolved one, are the collapsed states §6.3 never
  // records — partial, never a pass.
  if (status === 'absent_source') {
    if (parsed.rows !== undefined && parsed.rows !== null && (!Array.isArray(parsed.rows) || parsed.rows.length > 0)) {
      throw new Error(`readRecordedReconciliation: ${RECONCILIATION_FILENAME} carries rows against an absent source — the two §6.3 states were collapsed; re-earn the reconciliation`);
    }
    return parsed;
  }
  ...
  if (!Array.isArray(rows) || rows.length === 0) {
    throw new Error(
      `readRecordedReconciliation: ${RECONCILIATION_FILENAME} carries no reconciliation rows on a resolved record — §6.3's missing rows are not a neutral outcome; the record is partial and must be re-earned (reconcile the integration target again)`
    );
  }
  // The verdict vocabulary comes from the PINNED SNAPSHOT (§5.5/§6.3) when one is
  // readable on this bundle — the same authority buildReconciliationRows validated
  // against when the record was earned. A bundle whose snapshot cannot be read keeps
  // the structural checks; the checkpoint identity that consumes this record refuses an
  // unreadable snapshot upstream, so the read here only refuses what it can prove.
  let verdicts = null;
  try {
    verdicts = readSchemaSnapshot({ statePath }).schema?.plan_level?.reconciliation_verdicts ?? null;
  } catch {
    verdicts = null;
  }
  const seen = new Set();
  for (const row of rows) {
    if (!row || typeof row !== 'object') {
      throw new Error(`readRecordedReconciliation: ${RECONCILIATION_FILENAME} carries a malformed row (not an object) — re-earn the reconciliation`);
    }
    if (typeof row.section !== 'string' || row.section.trim() === '') {
      throw new Error(`readRecordedReconciliation: ${RECONCILIATION_FILENAME} carries a row with no section name — a partial row is re-earned, never consumed`);
    }
    if (seen.has(row.section)) {
      throw new Error(`readRecordedReconciliation: ${RECONCILIATION_FILENAME} carries duplicate rows for section ${JSON.stringify(row.section)} — re-earn the reconciliation`);
    }
    seen.add(row.section);
    if (typeof row.verdict !== 'string' || row.verdict.trim() === '') {
      throw new Error(
        `readRecordedReconciliation: ${RECONCILIATION_FILENAME} carries no verdict for section ${JSON.stringify(row.section)} — every repository INTENT.md section needs one (§6.3: a missing verdict is not a neutral outcome)`
      );
    }
    if (Array.isArray(verdicts) && verdicts.length > 0 && !verdicts.includes(row.verdict)) {
      throw new Error(
        `readRecordedReconciliation: ${RECONCILIATION_FILENAME} carries verdict ${JSON.stringify(row.verdict)} for section ${JSON.stringify(row.section)} — not in the pinned schema snapshot's reconciliation_verdicts (${verdicts.join(', ')}) — the record is unreadable in its own vocabulary, re-earn it`
      );
    }
    if (typeof row.artifact_path !== 'string' || row.artifact_path.trim() === '') {
      throw new Error(`readRecordedReconciliation: ${RECONCILIATION_FILENAME} carries no artifact_path on the row for ${JSON.stringify(row.section)} — a partial row is re-earned, never consumed`);
    }
    if (!/^sha256:[0-9a-f]{64}$/.test(String(row.artifact_digest))) {
      throw new Error(
        `readRecordedReconciliation: ${RECONCILIATION_FILENAME} carries a malformed section digest on the row for ${JSON.stringify(row.section)} — the per-section artifact binding is part of the record, never optional`
      );
    }
    const rr = row.resolved_ref;
    if (!rr || typeof rr !== 'object'
      || typeof rr.repository !== 'string' || typeof rr.remote !== 'string' || typeof rr.ref !== 'string'
      || !/^[0-9a-f]{40}$/.test(String(rr.resolved_commit))) {
      throw new Error(
        `readRecordedReconciliation: ${RECONCILIATION_FILENAME} carries a malformed resolved_ref on the row for ${JSON.stringify(row.section)} — the row's resolution anchor is part of the record, never optional`
      );
    }
  }
  return parsed;
