...
//
// THIS MODULE IS THE publish-time gate (C10, retain-intentionally). It is consumed ONLY by
// test/publish-hygiene.test.mjs — the live gate run on the release path (`node --test
// test/publish-hygiene.test.mjs` / full `npm test`). No runtime or doctor code imports it; the
// three detector families below are the release-safety bar, and the test file is their only driver.
//
...
// gitignored plugin manifests from it). Every tracked manifest version field must agree. As of the
// v8.0.0 release the dev marker (`8.0.0-ng.0`) is retired, so package.json is now INCLUDED by the
// caller and must agree with the README release like every other manifest.
