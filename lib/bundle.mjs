...
export function loadPlanTasks(state, planIndex) {
  ...
  const tasks = list.map((p) => {
    ...
    const id = /^-?\d+$/.test(String(rawId)) ? Number(rawId) : rawId; // numeric-string ids → Number (decide/markTask compare)
    const wave = p.wave ?? p.parallel_group;
    if (!Number.isInteger(wave)) {
      throw new Error(
        `loadPlanTasks: plan.index task ${JSON.stringify(rawId)} has a non-integer wave (${JSON.stringify(wave)}) — ` +
