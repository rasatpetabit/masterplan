...
export function buildSeedState(opts = {}) {
  const {
    ...
    ownerLock,
    codexReview = true, // Spec §4.1: default-on at seed (was: never emitted). Finish-step's
                        // defensive arm (lib/finish-step.mjs) covers legacy bundles missing it.
  } = opts;
  ...
  // Codex review config (nested state.codex.{routing,review}). The dispatch path reads
  // state.codex?.review (finish-step.mjs's codexArmed gate). When codexReview is true, persist the
  // explicit review:true so future audit re-entry guards can distinguish "review opted in" from
  // "review never configured" (the defensive-arm event in finish-step.mjs covers the latter).
  // When codexReview is false (explicit opt-out), the field stays absent — A9 absent-field style:
  // absent and explicit-false both yield codexArmed(undefined) === false at runtime, but the audit
  // trail differs (explicit opt-out vs. never-decided).
  if (codexReview === true) state.codex = { routing: 'auto', review: true };
