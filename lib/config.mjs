...
export const CONFIG_SCHEMA = {
  ...
  // The finish-gate fallback reviewer override (review-fallback): a list of model-ref
  // strings REPLACING the routing-policy-derived default list, or the scalar `off`
  // (no fallback — a failed primary review skips as it did before the fallback existed).
  // null (the default) means "derive the list from the routing policy"
  // (lib/dispatch/routing-policy.mjs adversaryFallbackReviewers). Validated fail-closed
  // by validateAdversaryReviewFallback below.
  adversary_review_fallback: { type: 'reviewer-ref-list|off', default: null },
...
/**
 * Validate an `adversary_review_fallback` value FAIL-CLOSED (review-fallback).
 *
 * Accepted: the scalar `'off'`, or a non-empty list of model-ref strings (which
 * REPLACES the routing-policy-derived default fallback list at the finish gate).
 * Rejected with a clear error: unknown scalars (including an explicit null/boolean/
 * number), non-string list entries, empty or whitespace-padded strings, and an empty
 * list (use `off` — an empty list is a typo, not a decision). Duplicates are
 * collapsed preserving first occurrence so dispatch order stays deterministic.
 *
 * @param {unknown} value
 * @returns {'off' | string[]} the normalized value
 */
export function validateAdversaryReviewFallback(value) {
  if (value === 'off') return 'off';
  if (!Array.isArray(value)) {
    throw new Error(`adversary_review_fallback must be the scalar 'off' or a list of model-ref strings, got ${JSON.stringify(value)} — a value that is neither disables the finish-gate fallback on a guess, so it fails closed`);
  }
  if (value.length === 0) {
    throw new Error("adversary_review_fallback list is empty — use the scalar 'off' to disable the finish-gate fallback, or list at least one model ref");
  }
  const out = [];
  for (const v of value) {
    if (typeof v !== 'string') {
      throw new Error(`adversary_review_fallback entries must be model-ref strings, got ${JSON.stringify(v)}`);
    }
    if (v.trim() === '') {
      throw new Error('adversary_review_fallback entries must be non-empty model refs — an empty string names no reviewer');
    }
    if (v !== v.trim()) {
      throw new Error(`adversary_review_fallback entry ${JSON.stringify(v)} carries surrounding whitespace — model refs are matched verbatim by the harness dispatch`);
    }
...
function resolveInterview(cli, repoData, userData, warnings) {
  ...
  const container = (label, data) => {
    if (data.interview === undefined) return null;
    // Fail closed on a malformed container (review finding, wave 13): an explicit null or a
    // non-object `interview:` value is NOT the absent case — silently defaulting it would
    // turn a broken config into the shipped minimum.
    if (data.interview === null || typeof data.interview !== 'object' || Array.isArray(data.interview)) {
      throw new Error(`interview is invalid in the ${label} .masterplan.yaml (it must be a mapping, e.g. interview: {probing_minimum: {medium: 3}}) — the probing minimum resolves fail-closed, never from a silently-defaulted container (spec §5.3)`);
    }
    if (data.interview.probing_minimum === null) {
      throw new Error(`interview.probing_minimum is explicitly null in the ${label} .masterplan.yaml — an explicitly-nulled minimum is a config error, not the absent case (spec §5.3 fail-closed)`);
    }
    return { value: { probing_minimum: validateProbingMinimum(data.interview.probing_minimum ?? DEFAULT_PROBING_MINIMUM) }, source: label };
  };
  return container('repo', repoData) ?? container('user', userData) ?? { value: { probing_minimum: Object.freeze({ ...DEFAULT_PROBING_MINIMUM }) }, source: 'default' };
}
...
export function resolveRunConfig({ cli = {}, repoRoot, home = os.homedir(), env = process.env } = {}) {
  ...
  // review-fallback: the finish-gate fallback reviewer override. null (no layer set it)
  // means "derive the default list from the routing policy" — resolved at the finish
  // gate (lib/finish-step.mjs), never here, so the list follows the policy the gate
  // itself dispatches on. Validated fail-closed: a malformed value is a config error.
  const fallbackPick = pick(cli, repoData, userData, 'adversary_review_fallback');
  values.adversary_review_fallback = fallbackPick.value !== undefined
    ? validateAdversaryReviewFallback(fallbackPick.value)
    : null;
  sources.adversary_review_fallback = fallbackPick.value !== undefined ? fallbackPick.source : 'default';
