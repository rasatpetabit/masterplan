...
export function validatePlanIndex(index, goals = []) {
  ...
  if (Array.isArray(goals) && goals.length > 0) {
    ...
    for (const g of goals) {
      // Truthiness, NOT `!== true`: parseGoals() sets `tombstone` to an OBJECT
      // ({reason, amended_at}), and bundle.mjs:376 rejects any non-object value. Every
      // other consumer (goals.mjs, finish-step.mjs, retro-goals.mjs) tests truthiness.
      // `!== true` was always satisfied by a real tombstone, so every tombstoned goal
      // was reported uncovered and no amended bundle could load its plan.
      if (!g.tombstone && !citedGoalIds.has(g.id)) {
...
export function renderPlanMd(index, meta = {}) {
  ...
  // The narrative meta (index.meta = {purpose, problem, solution}) is rendered here exactly as
  // plan.html renders it. plan.md is the canonical plan, so a stage stated only in the HTML
  // would be invisible to every reader who opens the markdown — which is most of them.
  const narrative = index && typeof index.meta === 'object' && index.meta && !Array.isArray(index.meta)
    ? index.meta : {};
