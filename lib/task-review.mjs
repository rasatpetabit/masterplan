...
export async function reviewCompletedTasks({
  ...
  const output = [];
  const deferred = [];
  // Per-item expected identity: an object (shared across tasks) or a function (item) => object.
  ...
  for (const item of items ?? []) {
    ...
    if (prior.present) {
      // Recovery re-entry is identity-gated: a legacy SHA-only event (no identity) can
      // never satisfy a recovery run, and a recovery event whose identity no longer
      // matches the CURRENT artifact is stale — both must re-review, never reuse.
      const identityOk =
        !requireIdentity ||
    ...
    let review;
    let status = 'done';
    let identity = requireIdentity ? identityFor(item) : null;
    try {
