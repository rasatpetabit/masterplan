...
export async function reviewCompletedTasks({
  statePath, runId, wave, baseSha, items, callReview, now = Date.now(), skillRoot,
  ...
  const output = [];
  const deferred = [];
  // Per-item expected recovery identity: an object (shared across tasks) or a function
  // (item) => object. Distinct from the §5.5 intent-identity tuple above — recovery
  // identity is the committed-diff artifact (base/head/diff_sha), stored on the event
  // via buildTaskReviewEvent's `identity` field.
  const identityFor = (item) => {
    if (typeof expectedIdentity === 'function') return expectedIdentity(item) ?? null;
  ...
  for (const item of items ?? []) {
    ...
    if (prior.present) {
      ...
      // Recovery re-entry is additionally identity-gated: a SHA-only event (no recovery
      // identity) can never satisfy a recovery run, and a recovery event whose identity
      // no longer matches the CURRENT artifact is stale — both must re-review, never reuse.
      const recoveryIdentityOk =
        !requireIdentity ||
        (prior.identity != null && identityFor(item) != null &&
          stableStringify(prior.identity) === stableStringify(identityFor(item)));
      if (identityCheck.ok && !reviewerUnavailable && recoveryIdentityOk) {
        const next = {
          ...item,
          review: prior.review,
      ...
      // Fall through to a fresh review: the stale, reviewer-unavailable, or recovery-
      // identity-mismatched receipt never suppresses one.
    }
    let review;
    let status = 'done';
    ...
    const recoveryIdentity = requireIdentity ? identityFor(item) : null;
    try {
    ...
    // Default: append immediately (existing native-review behavior). Recovery mode passes
    // onEvent so the append is DEFERRED until after the whole batch validates and the final
    // identity/ownership checks pass inside recordWaveResult — a post-append crash must not
    // become an unbound approval, and a failing receipt batch must not append anything.
    if (typeof onEvent === 'function') {
      deferred.push({ item, event, review });
    } else {
  ...
  if (typeof onEvent === 'function' && deferred.length) {
    onEvent(deferred);
  }
  return output;
}
