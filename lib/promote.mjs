...
function pinBase(exec, repoRoot, bundleId, transactionId, role, artifactPath) {
  ...
  //
  // The ref is NAMESPACED BY BUNDLE (gate iteration 4's finding): transaction documents and
  // duplicate-ID checks are bundle-local, so a repo-global ref keyed by the transaction ID
  // alone let two legitimate bundles using the same ID overwrite each other's GC protection —
  // the second pin orphans the first's pre-image and a gc makes the first transaction's
  // revalidation unrecoverable despite its untouched document. The bundle's slug (a bare-slug,
  // refname-safe by the SLUG_RE precedent) keeps every bundle's pins disjoint.
  if (!/^[a-z0-9][a-z0-9-]*$/.test(String(bundleId))) {
    throw new Error(`pinning base ${role}.md needs a refname-safe bundle id (the state's slug), got ${JSON.stringify(bundleId)} — refusing (§5.6)`);
  }
  ...
  const ref = `${PROMOTION_REFS_PREFIX}${bundleId}/${transactionId}/${role}-base`;
...
export function beginPromotion({
  ...
  const repoRoot = deriveRepoRoot(exec, statePath);
  const bundleId = readState(statePath).slug;
  const pins = {};
  for (const role of ROLES) {
    pins[role] = pinBase(exec, repoRoot, bundleId, transactionId, role, path.join(bundleDir, `${role}.md`));
  }
