...
// `satisfied`, never a torn state. Recovery completes the approved promotion, never
// rolls one back, never re-asks the operator. Recording is exactly-once by
// transaction_id: a replay that finds the record already present is a no-op.
...
// The decision table's classification for one artifact.
function classifyArtifact(diskDigest, base, result) {
  if (base === result) return 'satisfied'; // an unchanged artifact never contributes a torn state
...
function recoverBaseBytes(exec, repoRoot, role, pin, diskBytes, diskClass) {
  try {
    ...
    if (diskClass === 'base') return diskBytes;
...
export function commitPromotion({ statePath, transactionId, self, now, exec } = {}) {
  ...
  // Exactly-once replay: a transaction whose record already landed is a no-op, never a
  // second write.
  if (promotionEvent(loadEvents(statePath), transactionId)) {
    ...
    return { outcome: 'recorded', replay: true, transaction_id: transactionId, writes: [], appended: [] };
  }
...
function recoverFromDoc({ statePath, doc, self, now, exec, via }) {
  ...
  // Exactly-once: a recorded transaction replays as a no-op.
  if (promotionEvent(loadEvents(statePath), transactionId)) {
    if (doc.state !== 'recorded') {
      writeDoc(statePath, transactionId, { ...doc, state: 'recorded' });
    }
