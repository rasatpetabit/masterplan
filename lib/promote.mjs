...
export function pinnedGoalsEvidenceHash(statePath, goalsMdText) {
  ...
  if (!pin.pin && pin.repairable) {
    try {
      ...
      throw new Error(`the durable format pin is missing and could not be repaired from the capture history (${e.message}) — the goals lineage cannot hash under it`);
    }
  }
  if (!pin.pin) {
    throw new Error(
      `the durable format pin could not be resolved (${pin.repairable ? 'the repair from the capture history failed' : (pin.error ?? 'unknown pin state')}) — the goals lineage must hash under the SAME pin every consumer dispatches on (§6.1), never an unpinned default`
    );
  }
...
function pinBase(exec, repoRoot, transactionId, role, artifactPath) {
  // `hash-object -w --no-filters` writes the base bytes as a git object; `update-ref` pins it
  // against gc. A base pinned this way survives `git gc --prune=now` — the ref keeps the
  // object reachable, so the diff can always be re-applied to the true base. --no-filters is
  // load-bearing (gate iteration 6's finding): hash-object otherwise applies the repo's
  // clean/EOL filters (.gitattributes '*.md text eol=lf' normalizes a CRLF artifact to LF),
  // so the pinned blob would NOT be the approved on-disk bytes and every later revalidation
  // would refuse on a legitimate transaction ('the pinned base does not match the recorded
  // base hash'). The pin must be EXACT bytes — the same identity the approval bound.
  //
  // The ref is CONTENT-ADDRESSED (gate iterations 4+5's findings): the base's own oid is
  // part of the ref name, so pins are ADDITIVE — no two promotions can ever overwrite each
  // other's pre-image, whatever their bundle, slug, worktree, or transaction-ID collisions.
  // The iteration-4 shape (slug-namespaced refs) still let two linked worktrees of one repo
  // (the same slug, the shared object store) with the same transaction ID overwrite each
  // other's ref when their base bytes differed; content addressing makes that impossible:
  // different bases -> different oids -> different refs (both survive); the same base -> the
  // same ref with the same oid (update-ref is an idempotent no-op). No slug-derived gate is
  // needed at all — which also un-blocks the amendment path for every seed-accepted slug
  // charset, and a missing slug can no longer pass through as a literal.
  const oid = gitExec(exec, repoRoot, ['hash-object', '-w', '--no-filters', '--', artifactPath]);
  if (!/^[0-9a-f]{40}$/.test(oid)) {
    throw new Error(`pinning base ${role}.md failed: ${JSON.stringify(oid)} is not an object id`);
  }
  const ref = `${PROMOTION_REFS_PREFIX}${transactionId}/${role}-base-${oid}`;
