...
function pinnedGoalsEvidenceHash(statePath, goalsMdText) {
 ...
 * ledger claims). An artifact no longer at its approved result is accepted ONLY as a
 * recorded supersession — a LATER amendment event in the ledger names the current disk
 * bytes as its own recorded result — so a legitimate follow-on amendment never reads as
 * an unfinished promotion. Unreadable artifacts mean "not at the approved results"; the
 * normal flow owns the named unreadable-artifact failures.
 */
function commitPointReached(statePath, doc, events) {
  ...
  const bundleDir = path.dirname(path.resolve(statePath));
  for (const role of ROLES) {
    // An artifact the amendment never changed cannot be "not at its result" — its base
    // IS its result; only a drift moves it, and drift is the decision table's refusal.
    if (doc.base_hashes[role] === doc.result_hashes[role]) continue;
    let bytes;
    try {
    ...
    // Not at the approved result: accepted only when a LATER recorded amendment names the
    // CURRENT disk bytes as its own result — the ledger's lineage proves the write
    // completed and was legitimately superseded, never rewound.
    const supersededBy = role === 'goals'
      ? events.some((e, i) => i > evIndex && e?.type === 'goal_amended'
        && e.data?.new_goals_hash === pinnedGoalsEvidenceHash(statePath, bytes))
      : events.some((e, i) => i > evIndex && e?.type === 'spec_amended'
        && e.data?.transaction_id !== doc.transaction_id
        && e.data?.result_hashes?.spec === sha256Of(bytes));
    if (!supersededBy) {
      return { reached: false, event: ev, artifact: role, note: 'not at its approved result and not superseded' };
    }
  }
