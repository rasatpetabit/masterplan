...
//
// Every function here is PURE: no fs, no network, no LLM. Shell supplies gh/git JSON;
...
//
//   1. GitHub LABEL lifecycle — the `mp:*` labels
...
// ---------------------------------------------------------------------------
// A8 — Coordination default derivation (publish bootstrap, §7.1)
// ---------------------------------------------------------------------------

/**
 * refSafePlanHash(planHash) → string | null
 *
 * Normalizes a stored plan_hash into a git-ref-safe component. The index stamps
 * plan_hash as `sha256:<hex>` (bin/masterplan.mjs A3 parity), but a git ref name
 * cannot contain ':' — so the leading `<algo>:` prefix is stripped, leaving the
 * bare hex. sha256 is the only algorithm minted, so the hex alone is a lossless,
 * collision-free ref component. Returns null for a null/empty hash.
 *
 * This is the SINGLE point where the `sha256:`-prefix-vs-git-ref-name collision
 * is resolved; every consumer of contract_ref derives through computeCoordDefaults
 * (which calls this) rather than substituting the raw plan_hash.
 */
export function refSafePlanHash(planHash) {
  if (planHash == null) return null;
  const s = String(planHash).trim();
  if (!s) return null;
  const colon = s.indexOf(':'); // strip a leading "<algo>:" prefix (git refs forbid ':')
  return colon >= 0 ? s.slice(colon + 1) : s;
}

/**
 * computeCoordDefaults(slug, planHash) → { contract_ref, integration_branch }
 *
 * Pure derivation of the two coordination refs a publish bootstrap pins into
 * state.coordination (§7.1). This is the SINGLE source of truth for the ref
 * shape — the publish first-use provisioning, the `mp set-coord --bootstrap`
 * writer, and the follower's contract fetch must all agree with what this
 * returns.
 *
 *   - contract_ref:       mp-coord/<slug>/<ref-safe-plan-hash>  (null if no hash)
 *   - integration_branch: mp-int/<slug>
 *
 * planHash is normalized via refSafePlanHash (strips the `sha256:` prefix the
 * index stamps). When planHash is absent the contract_ref is null — a caller
 * must treat that as "not bootstrappable yet" (a contract ref with no plan hash
 * would be ambiguous and non-resumable), and must NOT pin a partial coordination.
 */
export function computeCoordDefaults(slug, planHash) {
  if (!slug) throw new Error('computeCoordDefaults: slug is required');
  const safeHash = refSafePlanHash(planHash);
  return {
    contract_ref: safeHash ? `mp-coord/${slug}/${safeHash}` : null,
    integration_branch: `mp-int/${slug}`,
  };
}
