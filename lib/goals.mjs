...
export function parseGoals(goalsMdText, opts = {}) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // 2.5 Intent section
    if (collectingIntent) {
      // Stop collecting when we hit any new heading
  ...
  return { topicSeed, goals: cleanedGoals, intent, version };
}
...
export function specAssumptionIds(specText) {
  ...
  // Code is not content. A fenced or indented block may quote table syntax as an EXAMPLE —
  // this spec quotes markdown in several places — and Markdown renders that as code, not as a
  // table. Reading it as one would let an example satisfy the coverage gate.
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (m) { fence = m[1][0].repeat(3); isCode[i] = true; continue; }
    if (/^ {4,}\S/.test(lines[i])) isCode[i] = true; // indented code block
  }
