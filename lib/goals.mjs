...
/** First non-empty line of a body, trimmed — the single-line legacy projection rule. */
...
function canonicalIntentObject(a) {
  const context = { outcome: { body: a.context.outcome.body }, done_means: { body: a.context.done_means.body } };
  for (const k of Object.keys(a.context).sort()) {
    if (k !== 'outcome' && k !== 'done_means') context[k] = { body: a.context[k].body };
  }
  return {
...
/** Raw `## Intent` block lines (unmasked), terminated by the same H1/H2 rule as the parser. */
function rawIntentBlockLines(goalsMdText) {
  const lines = goalsMdText.split(/\r\n|\n|\r/);
  let start = -1;
  for (let i = 0; i < lines.length; i++) {
    if (/^##\s+Intent\s*$/i.test(lines[i].trim())) { start = i + 1; break; }
  }
  if (start === -1) return null;
  const out = [];
  for (let i = start; i < lines.length; i++) {
    if (/^#{1,2}\s+/i.test(lines[i].trim())) break;
    out.push(lines[i]);
  }
  return out;
}
...
/**
 * Find the fenced codec blocks inside a raw block. A fence whose info string carries
 * the codec marker (prefix `mp-intent-schema`) is COLLECTED whatever its version —
 * an unknown or malformed version marker must be rejected by the caller, never
 * silently skipped as ordinary quoted code. Any other fence is ordinary code.
 * Returns [{info, content}].
 */
function findCodecFences(blockLines) {
  const FENCE = /^ {0,3}(`{3,}|~{3,})(.*)$/;
  const found = [];
  let open = null; // { char, len, info, content: [] }
  for (const line of blockLines) {
    const m = FENCE.exec(line);
    if (open) {
      if (m && m[1][0] === open.char && m[1].length >= open.len && m[2].trim() === '') {
        found.push({ info: open.info, content: open.content.join('\n') });
        open = null;
      } else {
    ...
    if (m) {
      const info = m[2].trim();
      if (m[1][0] === '`' && info.split(' ')[0] === INTENT_CODEC_FENCE_INFO) {
        open = { char: m[1][0], len: m[1].length, info, content: [] };
      }
      // Any other fence inside the Intent block is ordinary quoted code — skipped.
    }
  }
  return found;
}
...
export function decodeIntent(goalsMdText) {
  ...
  const blockLines = rawIntentBlockLines(goalsMdText);
  if (!blockLines) return { versioned: false, authoritative: null, legacyView };
  const fences = findCodecFences(blockLines);
  ...
  let obj;
  try {
    obj = JSON.parse(fences[0].content);
  } catch (e) {
