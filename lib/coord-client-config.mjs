...
//
// Design (names/contracts derived from the established agent-dispatch reference
// packages/core/coord-client-config.mjs, read-only):
...
export function applyCoordPathAutodiscover(baseEnv = process.env, seams = {}) {
  ...
  if (!Object.prototype.hasOwnProperty.call(env, 'COORD_CREDS_FILE')) {
    const creds = join(coordDir, 'creds.json');
    if (pathExists(creds)) env.COORD_CREDS_FILE = creds;
  }
  if (!Object.prototype.hasOwnProperty.call(env, 'COORD_TLS_CA')) {
...
export function resolveCoordClientConfig({
  ...
  const credsFile = resolved.COORD_CREDS_FILE;
  ...
  const caPath = resolved.COORD_TLS_CA || null;
  let ca = null;
  if (caPath) {
    try {
      ca = readFile(caPath);
