...
export function selectLaunchPath({ codexSuppressed = false, nativeSpawn = null, env = process.env } = {}) {
  ...
  // A genuine Codex host has no native parallel spawn API — MCP pool regardless of the
  // env flag (the flag cannot conjure an API that does not exist). This veto does NOT
  // fire on Pi: Pi sets codexSuppressed=true (PI_CODING_AGENT) but also
  // hostHasNativeSpawnApi=true (same env var), so the condition is false and Pi falls
  // through to auto-detection below.
  if (codexSuppressed && !hostHasNativeSpawnApi(env)) return 'mcp-pool';
  // Explicit env override (any host that survived the Codex veto):
  // 1/true/on → native-spawn, 0/false/off → mcp-pool (opt-out on Pi).
  const flag = String(env.MP_DISPATCH_NATIVE_SPAWN ?? '').trim().toLowerCase();
  if (flag === '1' || flag === 'true' || flag === 'on') return 'native-spawn';
  if (flag === '0' || flag === 'false' || flag === 'off') return 'mcp-pool';
  // Pi (and any host with a native parallel spawn API) defaults to native-spawn — the
  // ONE host that can run it should not need an env flag to reach its own API. The prior
  // default of 'mcp-pool' made the native branch unreachable in production (e2e finding
  // 1, test/e2e-native-wave-report.md): every wave silently went through the MCP broker.
  if (hostHasNativeSpawnApi(env)) return 'native-spawn';
  // Claude Code and other hosts without a native API keep the MCP pool.
  return 'mcp-pool';
}
