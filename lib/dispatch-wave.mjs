...
// the L1 op table listed only dispatch_fabric / dispatch_fabric, so masterplan
...
function readAgentRole(taskClass, { bin = 'agent-dispatch', _exec = execFileSync } = {}) {
  if (AGENT_MAPPING_CACHE === null) {
    ...
    try {
      ...
      AGENT_MAPPING_CACHE = JSON.parse(stripped).agent_mapping ?? {};
    } catch {
...
function renderSpawnPrompt(task, desc, token) {
  ...
  return [
    ...
    '',
    'Report concrete evidence. Commit locally in your locus; do not push.',
  ].join('\n');
}
...
export function selectLaunchPath({ codexSuppressed = false, nativeSpawn = null, env = process.env } = {}) {
  ...
  if (codexSuppressed) return 'mcp-pool'; // Codex host: §0 host-detect, no native API either
...
/** L2 verdict vocabulary (workflows/dispatch-wave extractVerdict). */
...
export async function dispatchWaveViaFabric({
  ...
  const scopePaths = run?.scope ?? tasks.flatMap((t) => t.files ?? []);
  ...
  try {
    ...
    try {
      ...
      // MCP dispatch_fanout is RETIRED (2026-07-17 cutover). Drive the wave as a
  ...
  for (let i = 0; i < digests.length; i++) {
    ...
    const rawCmds = Array.isArray(tasks[i]?.verify_commands) ? tasks[i].verify_commands : [];
    if (rawCmds.length === 0) {
