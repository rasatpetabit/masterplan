...
//                     remaining starts attempt N+1 (a recover_wave retry is
...
//      (agent-dispatch serve-mcp) driving a bounded concurrent pool of per-task
//      `dispatch_task` calls (isolated fail mode), instead of N per-task spawns. Each
...
//      diff (never scope-filtered) is reviewed via explicit dispatch_review on the
//      open broker client (injectable via _callReview). Lifecycle/persistence is
//      owned by lib/task-review.mjs; verdict rides in digest.review / item.review
//      → blocking_reviews[] in the record protocol; idempotency via run+task+sha
//      (sha = sha256 of the review payload, so changed code at the same HEAD re-arms).
...
//      the dispatch_fabric / dispatch_fabric paths: mark digests → D6
...
// Behavior-preserving: the orchestrator sequences these and keeps the acquire /
// launch / broker / finalize half inline until a later extraction.
// ---------------------------------------------------------------------------
...
/**
 * Plan-index read, routing-input parity, prepareWave, worktree/MAIN, fingerprint.
...
export async function dispatchWaveViaFabric({
  ...
  // 7d. LAUNCH-PATH BRANCH. On a native-spawn host the wave leaves this module as a
  //     PLAN — the harness owns child spawning, so `mp` returns descriptors instead of
  //     executing them, and the caller appends handles (phase 2) via `mp promote-run`.
  //     The record is already durable with the token, so a crash between here and the
  //     caller's spawn is recoverable by probeWaveToken, not by re-dispatch.
  if (selectLaunchPath({ codexSuppressed, nativeSpawn }) === 'native-spawn') {
    const plan = buildNativeSpawnPlan({ tasks, descriptors, token: waveToken, bin: brokerBin });
