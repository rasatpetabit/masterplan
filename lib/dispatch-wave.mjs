...
export function resolveClassRouting(taskClass, { policy = null, _cache = CLASS_ROUTING_CACHE } = {}) {
  ...
  const cacheKey = `${taskClass}\u0000${policy ? 'injected' : (readEnv('MP_ROUTING_POLICY') || 'repo')}`;
...
export function normalizeWaveConcurrency(requested, taskCount) {
  const envRaw = Number(readEnv('MP_DISPATCH_WAVE_CONCURRENCY'));
 ...
 *     approve.
 *
 * COMMITTED-RECOVERY MODE (recoverySelector supplied): the recovery controller
 * is a separate explicit path for resuming a wave whose work was already
 * committed to a recovered HEAD (clean tree, frozen base). It validates the
 * explicit selector (expected repo + full HEAD) against the frozen
 * review_context, captures the deterministic base→HEAD diff, and requires every
 * Phase B receipt to carry the full recomputed identity. In this mode review
 * events are DEFERRED until recordWaveResult (identity/ownership validated).
 *
 * @param {{ statePath: string, result: object, providedReviews?: Object<string,object>|null,
 *           policy?: object|null, now?: number, skillRoot?: string,
 *           recoverySelector?: {repo: string, head: string}|null }} opts
 */
export async function reviewNativeResult({
  statePath, result, providedReviews = null, policy = null, now = Date.now(), skillRoot,
  recoverySelector = null,
} = {}) {
  ...
  if (!ctx?.enabled) {
    // Explicit recovery with absent/disabled review context FAILS (never the legacy no-op).
    if (recoverySelector) {
      throw new Error('recovery: review_context is absent or disabled — committed recovery requires the frozen review context (explicit recovery must fail, not take the legacy no-op)');
    }
    return result;
  }

  const runId = String(state.slug ?? record?.run_id ?? '');
  const contextFingerprint = fingerprintReviewContext(ctx);

  // ----- COMMITTED-RECOVERY MODE ----------------------------------------------
  if (recoverySelector) {
    return reviewCommittedRecovery({
      absState, state, bundleDir, record, ctx, runId, wave,
      contextFingerprint, result, providedReviews, policy, now, skillRoot,
      selector: recoverySelector,
    });
  }

  // ----- NORMAL WORKING-DIFF MODE (unchanged behavior) -------------------------
...
/**
 * Committed-recovery mode body. Validates the explicit selector against the frozen
 * context, captures the deterministic base→HEAD artifact, and binds every receipt.
 * Returns a result whose `deferred_review_events` must be passed to recordWaveResult;
 * events are NOT appended here.
 */
export async function reviewCommittedRecovery({
  absState, state, bundleDir, record, ctx, runId, wave,
  contextFingerprint, result, providedReviews, policy, now, skillRoot,
  selector,
} = {}) {
  const bundle = String(state.slug ?? record?.run_id ?? '');
  const attempt = Number.isInteger(record?.attempt) ? record.attempt : null;

  // 2. Validate the explicit selector + frozen context before capture.
  if (!selector || typeof selector !== 'object') {
    throw new Error('recovery: recoverySelector is required in recovery mode');
  }
  const { repo: expectedRepo, head: expectedHead } = selector;
  if (typeof expectedRepo !== 'string' || expectedRepo.trim() === '') {
    throw new Error('recovery: selector.repo must be a non-empty path');
  }
  if (typeof expectedHead !== 'string' || !/^[0-9a-f]{40}$/.test(expectedHead)) {
    throw new Error('recovery: selector.head must be a full 40-hex commit OID (never derive or accept short OIDs)');
  }

  // Single edit locus: the frozen task set must name exactly one repository, and the
  // canonical identity of that repo must agree with the selector (reject mixed loci).
  const frozenRepos = [...new Set((ctx.tasks ?? []).map((t) => t?.repo).filter(Boolean))];
  if (frozenRepos.length !== 1) {
    throw new Error(`recovery: frozen review_context names ${frozenRepos.length} repositories — recovery supports exactly one edit locus, got ${frozenRepos.join(', ')}`);
  }
  const repo = path.resolve(frozenRepos[0]);
  if (path.resolve(expectedRepo) !== repo) {
    throw new Error(`recovery: selector.repo ${path.resolve(expectedRepo)} does not match the frozen review_context repo ${repo}`);
  }
  const identity = canonicalRepoIdentity(repo);
  if (identity.name !== path.basename(repo)) {
    throw new Error(`recovery: cannot resolve canonical repo identity for ${repo}`);
  }

  // Full HEAD equality + valid commit objects + base ancestry.
  const head = resolveCommitOid(repo, expectedHead);
  if (head !== expectedHead) {
    throw new Error(`recovery: selector.head ${expectedHead} resolves to ${head} — exact HEAD equality required`);
  }
  const base = resolveCommitOid(repo, ctx.base_sha);
  if (!isAncestor(repo, base, head)) {
    throw new Error(`recovery: frozen base ${base} is not an ancestor of head ${head} — unrelated ancestry rejected`);
  }
  if (base === head) {
    throw new Error('recovery: frozen base equals head — the committed delta is empty; nothing to review');
  }

  // Clean tree required (recovery is a committed state — working residue is a drift risk).
  if (!probeCleanTree(repo)) {
    throw new Error(`recovery: ${repo} is not clean (index/tracked/untracked dirty) — committed recovery requires a clean tree`);
  }

  // 3. Deterministic capture: HEAD + cleanliness read BEFORE capture, then capture, then
  //    re-read HEAD + cleanliness AFTER — drift fails.
  const headBefore = resolveCommitOid(repo, 'HEAD');
  const cleanBefore = probeCleanTree(repo);
  if (!cleanBefore) {
    throw new Error('recovery: tree became dirty between the pre-capture clean probe and capture — aborting');
  }
  if (headBefore !== head) {
    throw new Error(`recovery: live HEAD ${headBefore} does not match the pinned head ${head} — refusing to capture a different commit`);
  }
  const diff = captureCommittedDiff(repo, base, head);
  const diffSha = sha256hex(diff);
  const headAfter = resolveCommitOid(repo, 'HEAD');
  const cleanAfter = probeCleanTree(repo);
  if (headAfter !== head || !cleanAfter) {
    throw new Error('recovery: HEAD moved or tree became dirty during capture — artifact is not trustworthy');
  }

  // Task-set validation (unknown/duplicate tasks, item↔digest disagreement).
  validateRecoveryTaskSet(ctx, result.tasks ?? []);

  // 4. Phase A manifest — one identity per task, carried into every review request.
  const expectedById = new Map();
  const items = (result.tasks ?? []).map((item) => {
    const taskId = item?.task_id ?? item?.digest?.task_id;
    const expected = buildRecoveryIdentity({
      bundle, runId, wave, attempt, contextFingerprint, taskId,
      repo, base, head, diffSha,
    });
    expectedById.set(String(taskId), expected);
    return {
      ...item,
      task_id: taskId,
      review_input: {
        ...
        base,
        head,
        diff: diff,       // the shared base→HEAD artifact (whole-wave committed delta)
        sha: diffSha,     // sha of the SAME artifact bytes all tasks review
  ...
  // Phase A — emit identity-bearing recovery descriptors.
  // NOTE: in recovery mode, caller-supplied item.review/digest.review is NEVER authority.
  // recoveryDone recomputes over ALL done tasks (embedded reviews are discarded) so a
  // smuggled approve can never satisfy coverage — only a binding receipt (or an already
  // identity-matched durable event, checked inside reviewCompletedTasks) counts.
  const recoveryDone = items.filter((it) => it?.digest?.status === 'done');
  if (providedReviews == null) {
    if (recoveryDone.length === 0) return { ...result, tasks: items };
    const adversary = resolveWorkClass('adversary', policy ? { policy } : {});
    const pending = recoveryDone.map((it) => {
      const captured = Buffer.isBuffer(it.review_input.diff)
        ? it.review_input.diff
        : Buffer.from(it.review_input.diff ?? '');
      return {
        task_id: it.task_id,
        class: 'adversary',
        agent: adversary.agent,
        ...
        effort: adversary.effort,
        repo: it.review_input.repo,
        base: it.review_input.base,
        head: it.review_input.head,
        // Lossless JSON carrier: NEVER put a Buffer here — JSON.stringify would emit
        // `{type:'Buffer',data:[...]}` and the reviewer would not see the artifact bytes.
        diff: encodeRecoveryDiff(captured),
        diff_encoding: 'base64',
        diff_sha: it.review_input.sha,
        format: RECOVERY_CAPTURE_FORMAT,
        identity: it.review_input.identity,
        job_id: `${runId}-w${wave}-t${it.task_id}-${it.review_input.sha.slice(0, 12)}`,
      };
    });
    return {
      ...
      tasks: items,
      pending_reviews: pending,
      review_outcome: 'recovery-review-pending',
    };
  }

  // Phase B — whole-batch validation BEFORE any event append: validate every provided
  // receipt's identity, then defer the events to recordWaveResult (which re-checks
  // ownership/epoch and appends only after the transaction guards pass).
  // Uses recoveryDone (ALL done tasks) — embedded reviews never exempt a task.
  const provided = new Map(Object.entries(providedReviews).map(([k, v]) => [String(k), v]));
  const errors = [];
  for (const it of recoveryDone) {
    const key = String(it.task_id);
    const rec = provided.get(key);
    if (rec == null) {
      errors.push(`native adversarial review not provided for task ${key}`);
      continue;
    }
    const bound = validateRecoveryReceipt(rec, expectedById.get(key));
    if (!bound.ok) {
      errors.push(`task ${key}: ${bound.error}`);
    }
  }
  if (errors.length) {
    throw new Error(`recovery: receipt binding failed — ${errors.join('; ')}`);
  }
  ...
  const reviewed = await reviewCompletedTasks({
    ...
    expectedIdentity: (item) => expectedById.get(String(item?.task_id ?? item?.digest?.task_id)) ?? null,
    callReview: async (args) => {
      const taskId = jobIdToTask(args.job_id, recoveryDone, runId, wave) ?? String(args.task_id ?? '');
      const rec = provided.get(taskId);
      if (rec == null) {
        throw new Error(`recovery: native adversarial review not provided for task ${taskId}`);
      }
      return rec;
    },
    onEvent: (events) => { deferredEvents = events; },
    now,
  });
  ...
  return {
    ...result,
    tasks: reviewed.map(({ review_input, ...item }) => item),
    review_outcome: 'recovery-reviews-recorded',
    deferred_review_events: deferredEvents,
  };
}

/** Local job-id → task-id resolver (mirrors the normal mode's map). */
function jobIdToTask(jobId, doneUnreviewed, runId, wave) {
  for (const it of doneUnreviewed) {
    const sha = it?.review_input?.sha?.slice(0, 12);
    if (sha && jobId === `${runId}-w${wave}-t${it.task_id}-${sha}`) return String(it.task_id);
  }
  return null;
}
...
export async function dispatchWaveViaFabric({
  ...
  const callerAllowlist = readEnv('SKYNET_VERIFY_ALLOWLIST');
