...
export function finalizeRecord({
  ...
  // 9. Results durable BEFORE the record transaction: a crash between here and
  //    record completion re-drives record-result from THIS result — nothing is
  //    re-launched for this attempt.
  const result = {
    wave,
  ...
  if (recordResult?.outcome === 'recorded') {
    ...
    // keep 'dispatched' (with the error) so a retry re-drives record, never re-launch.
    current = writeWaveDispatchRecord(bundleDir, wave, {
      ...current,
