...
export function resolveConfigDir(env, home = os.homedir()) {
  const override = (readEnv('CLAUDE_CONFIG_DIR', env) ?? '').trim();
...
export function resolveMasterplanBin(env, home = os.homedir()) {
  const mpBin = (readEnv('MP_BIN', env) ?? '').trim();
  if (mpBin !== '') return path.isAbsolute(mpBin) ? mpBin : path.join(home, mpBin);
  const mpMarketplaceDir = (readEnv('MP_MARKETPLACE_DIR', env) ?? '').trim();
...
export function resolveRunsDir(repoRoot, env) {
  const override = (readEnv('MASTERPLAN_RUNS_DIR', env) ?? '').trim();
  const raw = override !== '' ? override : 'docs/masterplan';
  return path.isAbsolute(raw) ? raw : path.join(repoRoot, raw);
}

export function resolveBundleDir(repoRoot, slug, env) {
  return path.join(resolveRunsDir(repoRoot, env), slug);
}

export function resolveStatePath(repoRoot, slug, env) {
...
export function bundleArtifacts(repoRoot, slug, env) {
