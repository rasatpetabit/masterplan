...
//
// Pure wave-preparation, scope-verification, and launch-context helpers (originally two functions bracketing the L2 Workflow engine; now six exports) that keep ALL decidable logic in tested L1 code
...
//     and emits a LEAN per-task payload carrying the dispatch `class` (routing is resolved
//     downstream from the policy, not pre-baked here — C6).
...
//
//   buildWaveLaunchContext({ state, planIndexPath, wave, routingInputs, reposAllowlist?, _exec? })
//     -> { prepared, planIndex, MAIN }
//     Shared PREPARE/EXECUTE launch-context seam: plan-index read, config/env from injected
//     routingInputs, prepareWave, MAIN from bundleDir. Callers own routing-input source selection.

// Routing/backend/eligibility decisions live in lib/dispatch/ (the consolidated dispatch
...
import { resolveTaskBackend, qctlEligible } from './dispatch/index.mjs';
...
import { computeTaskSpecHash, computeInputFingerprint, composeHandoffKey } from './fabric-idempotency.mjs';
...
// against. Shell-side by design (fabric-idempotency is pure); git is -C-qualified and injectable
// (`_exec`) following the runGit idiom in watch-integrity.mjs. Deterministic for an unchanged tree:
...
// What the workflow actually needs per task. NEVER spec excerpts or
// file contents — this payload transits the orchestrator context (goal 3).
// (C6: the legacy pre-resolved target/eligible/reason routing payload was deleted —
// routing is policy-resolved from the task class downstream.)


// ---- fabric-mode routing (strangler phase flag: state.dispatch.fabric / config.fabric) --------
//
// Routing/eligibility no longer runs through masterplan's DUPLICATED routing brain (the
// pre-resolved target/backend decision modules were deleted in C6). Each task carries only its
// dispatch `class`; the routing-policy resolver (lib/dispatch/routing-policy.mjs) maps that
// class to its governed agent role + lane model — one routing brain, not two. This is the
// SAME strangler phase flag that gates the wave dispatch op (buildWaveDispatchOp's `fabric`
// branch, sourced from state.dispatch.fabric).
//
// A2 (2026-08-30): the former default `masterplan-implementation` is absent from policy/workflow-map.json and
// fell back to `workflow.defaultClass: "unknown"`. Repointed to the existing `bounded-edit` class
// (agent `builder`, writes: true, cap: `edit` — the implementer semantic). Routing is neutral —
// `bounded-edit` carries the same agent/model chain as the effective `unknown` fallback, so no
// in-flight bundle changes lane; the spawn-record `capability` field does change chat→edit, but it
// is recorded only (no gating consumer reads it).
...
// otherwise the masterplan-worker default (the policy-overlay-governed implementer class). NO codex/inline/backend
...
// The lean fabric payload: only what the seam adapter consumes. NO target/eligible/reason —
// those are resolved downstream by core resolve/guard from the task `class`, not pre-baked here.
// The ONE exception is the dormant qctl `backend` discriminator, attached by prepareWave
// (not here) only when the qctl flag is armed AND the task is eligible — see prepareWave below.
...
// hashes only, never raw diffs) computed via the pure fabric-idempotency module, and the wave result
...
export function prepareWave(state = {}, planIndex = {}, wave, config = {}, env = {}, reposAllowlist, dispatchInputs) {
  ...
  // branch reads state.dispatch?.fabric). Fabric is the ONLY wave path (C6 deleted the
  // pre-resolved target/backend payload; dispatch-wave fails closed on a non-fabric
  // bundle), so every task carries the class-only fabric payload unconditionally.
  ...
  const tasks = pending.map((st) => {
    ...
    // Merge for the qctl seam: state owns {id,wave,status,files}; plan.index owns the exec
    // fields resolveTaskBackend reads (files/description/verify_commands/sensitive).
    const merged = {
      files,
    ...
    // Routing is deferred to core resolve/guard via the seam — carry only the class.
    const payload = fabricPayload(st, p, resolveTaskClass(p), files);
    // Dormant qctl seam (docs/design/qctl-multi-repo-apply.md): when the qctl discriminator
    // is armed (config.implementer.qctl.enabled === true — the same strictly-true gate
    // resolveTaskBackend itself short-circuits on), the §6.3 eligibility gate
    // (resolveTaskBackend/qctlEligible, lib/dispatch/backend.mjs) is exercised from HERE — so
    // the seam stays reachable after the routing brain deletion. An eligible task's payload
    // gains the `backend` work-item discriminator; {kind:'agent'} (the ineligible downgrade
    // AND the shipped flag-off default) is OMITTED ENTIRELY, so payloads and handoff keys for
    // tasks that don't select qctl stay byte-identical by construction (like the descriptor's
    // `review` field, `backend` is advisory/descriptor-only and never hashed).
    if (config?.implementer?.qctl?.enabled === true) {
      const backend = resolveTaskBackend(merged, config, env, reposAllowlist);
      if (backend.kind !== 'agent') payload.backend = backend;
    }
    return payload;
  });
...
// Shared PREPARE/EXECUTE launch-context seam: plan-index read, config/env derivation from
// INJECTED routingInputs, prepareWave, and MAIN resolution from the bundle dir. Callers own
// routing-input source selection (persisted record vs current flags); this function never
// reads state.codex.routing or host flags itself. reposAllowlist is optional — the fabric
// path omits it (policy-resolved routing); PREPARE may pass it for legacy multi-repo eligibility.
export function buildWaveLaunchContext({
  ...
  let MAIN;
  try {
    MAIN = path.dirname(String(_exec(
      'git',
      ['-C', bundleDir, 'rev-parse', '--path-format=absolute', '--git-common-dir'],
      { encoding: 'utf8' },
    )).trim());
  } catch {
...
// ---- the mid-run goals reminder (§6.2) ---------------------------------------
//
// The run's frozen goals are the bar the finish measures against, so every wave summary
// carries them forward. For a v2 document that means the Intent's `outcome:` line —
// VERBATIM, because a paraphrase is a different bar — and for a v1 document, which has no
// Intent block, the goal list it does have.

/**
 * The RAW `outcome:` source line from the Intent block, exactly as the frozen document wrote
 * it. The parsed value is normalized (trimmed, split off its key), so quoting it back would
 * be a reconstruction, not a quotation — and the outcome is the operator's own statement of
 * what is true in the world when the run is done, which is the one thing a reminder must not
 * reword. The scan is bounded to the Intent block so an `outcome:` line belonging to a goal
 * further down the document can never be picked up instead.
 *
 * @param {string} goalsMd
 * @returns {string|null} the source line, or null when the Intent block has no outcome line
 */
export function rawIntentOutcomeLine(goalsMd) {
  if (typeof goalsMd !== 'string') return null;
  // Delegated to the PARSER, which captures the source line of the very field it assigned. A
  // second scanner here would be a second set of lexical rules — where a fence ends, which
  // heading closes the Intent block, which of two `outcome:` lines wins — and the two would
  // drift silently until the reminder quoted something the verdict was never judged against.
  try {
    return parseGoals(goalsMd).intentOutcomeLine ?? null;
  } catch {
    return null;
  }
}
...
/**
 * The one-line goals reminder a wave summary carries.
 *
 * @param {string|null} goalsMd - The bundle's goals.md, or null when it has none.
 * @returns {{version: 1|2|null, line: string|null, outcome: string|null, goals: string[]}}
 */
export function goalsReminder(goalsMd) {
  if (typeof goalsMd !== 'string' || goalsMd.trim() === '') {
    return { version: null, line: null, outcome: null, goals: [] };
  }
  let parsed;
  try {
    parsed = parseGoals(goalsMd);
  } catch {
    // An unreadable goals document must not take down a wave summary: the reminder is a
    // courtesy, and the finish-time goal check is what actually enforces the bar.
    return { version: null, line: null, outcome: null, goals: [] };
  }
  const goals = (parsed.goals ?? []).filter((g) => g && !g.tombstone).map((g) => `${g.id}: ${g.text}`);
  const goalsLine = goals.length ? `goals: ${goals.join('; ')}` : null;
  if (parsed.version === 2) {
    // The RAW line — see rawIntentOutcomeLine. The parsed value travels alongside it for
    // callers that want the semantic string rather than the source text.
    const raw = rawIntentOutcomeLine(goalsMd);
    const outcome = typeof parsed.intent?.outcome === 'string' && parsed.intent.outcome.trim()
      ? parsed.intent.outcome
      : null;
    // A v2 document whose Intent carries no outcome line still has its goals.
    return { version: 2, line: raw ?? goalsLine, outcome, goals };
  }
  // v1: no Intent block exists to quote, so the goal list is the reminder.
  return { version: 1, line: goalsLine, outcome: null, goals };
}

/**
 * Reads the bundle's goals.md, if it has one, and returns its reminder. A bundle with no
 * goals document is not an error — goals are optional.
 *
 * @param {string} bundleDir
 * @returns {{version: 1|2|null, line: string|null, outcome: string|null, goals: string[]}}
 */
export function bundleGoalsReminder(bundleDir, _fs = fs) {
  try {
    return goalsReminder(_fs.readFileSync(path.join(bundleDir, 'goals.md'), 'utf8'));
  } catch {
    return { version: null, line: null, outcome: null, goals: [] };
  }
}
...
/**
 * The terse wave summary the shell prints: what completed, what is next, and the goals
 * reminder. Deliberately one or two lines — a wave summary that floods the screen is the
 * thing the anti-flood rule exists to prevent.
 *
 * @param {{wave:number, recorded?:number[], failed?:Array, next?:string|null, bundleDir?:string, goalsMd?:string|null}} input
 * @returns {{lines: string[], reminder: object}}
 */
export function waveSummary({ wave, recorded = [], failed = [], next = null, bundleDir = null, goalsMd = null } = {}) {
  const reminder = goalsMd !== null ? goalsReminder(goalsMd) : (bundleDir ? bundleGoalsReminder(bundleDir) : goalsReminder(null));
  const lines = [];
  const done = recorded.length ? `tasks ${recorded.join(', ')}` : 'no tasks';
  const failedPart = failed.length ? `, ${failed.length} failed` : '';
  lines.push(`wave ${wave}: recorded ${done}${failedPart}${next ? ` — next: ${next}` : ''}`);
  if (reminder.line) lines.push(reminder.line);
