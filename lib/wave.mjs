...
// class to its governed agent role + lane model — one routing brain, not
...
// those are resolved downstream by core resolve/guard, not pre-baked here. The ONE exception is
// the dormant qctl `backend` discriminator, attached by prepareWave's fabric branch (not here)
// only when the qctl flag is armed AND the task is eligible — see the fabric branch below.
...
export function prepareWave(state = {}, planIndex = {}, wave, config = {}, env = {}, reposAllowlist, dispatchInputs) {
  ...
  const tasks = pending.map((st) => {
    ...
    if (fabric) {
      ...
      // (resolveTaskBackend/qctlEligible, lib/dispatch/backend.mjs) is exercised from THIS
      // fabric branch — so the seam stays reachable after the legacy routing brain below is
      // deleted. An eligible task's payload gains the `backend` work-item discriminator;
      // {kind:'agent'} (the ineligible downgrade AND the shipped flag-off default) is OMITTED
      // ENTIRELY, so payloads and handoff keys for tasks that don't select qctl stay
      // byte-identical by construction (like the descriptor's `review` field, `backend` is
      // advisory/descriptor-only and never hashed).
