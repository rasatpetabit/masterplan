...
// ---- refs on-load default: absent refs -> {back:[],forward:[]}; present refs preserved ----
test('migrate(6.x): an absent refs key loads as the empty-refs default', () => {
  const text = 'schema_version: 6\nslug: x\nstatus: executing\nphase: C\n';
  assert.deepEqual(migrate(text).refs, { back: [], forward: [] });
});
test('migrate(6.x): an existing refs value is preserved untouched', () => {
  const v8 = { schema_version: '6.0', slug: 'demo', pending_gate: null, active_run: null,
               refs: { back: ['a'], forward: ['b'] },
               tasks: [{ id: 1, status: 'done', wave: 0, files: ['a.js'] }] };
  const text = serializeState(v8);
  assert.deepEqual(migrate(text).refs, { back: ['a'], forward: ['b'] });
});
