...
test('goals: archived bundle with event-level ts and null state.goals re-validates record-goal-check against goals.md', () => {
  // Regressions: record-goal-check stores ts at the event level, and older/in-flight bundles may keep
  ...
  const goalCheckEvent = {
    ...
    data: {
      ...
      verdicts: {
        G1: { verdict: 'achieved', evidence: 'done' },
        G2: { verdict: 'achieved', evidence: 'done' },
      },
  ...
  const findings = goals(tmp);
  assertFindingShape(findings);
  assert.equal(maxSeverity(findings), 'PASS', 'event-level-ts/null-cache goal_check must re-validate PASS, not ERROR — ' + JSON.stringify(findings));
  fs.rmSync(tmp, { recursive: true, force: true });
});
