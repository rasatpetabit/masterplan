...
*   **The Context Degradation Loop:** As an inline session grows, it continuously accumulates stale code drafts, build logs, and past conversations. This clutter degrades model reasoning in two distinct ways:
    ...
    2.  *Compounding Cost (Token Efficiency):* Every single accumulated byte is rebilled on every single turn. A long inline run pays compound interest on thousands of tokens of context that should have been discarded hours ago, multiplying your API costs exponentially.
...
*   **Guard C (flock):** A concurrency lock (`.lock` file with a 5-second write timeout managed via `bin/masterplan-state.sh`). This prevents concurrent workflows or multi-agent worktrees from corrupting `state.yml` or `events.jsonl`. Doctor check #42 warns when `.lock` is older than 1 hour. *(See [parts/step-c.md](./parts/step-c.md))*
...
*   `/masterplan doctor [--fix]`
    *   **Phase:** Diagnostics
    *   **What it does:** Runs 43 proactive lint checks across all plan bundles to catch state drift. `--fix` resolves repairable issues.
*   `/masterplan status`
    *   **Phase:** Diagnostics
