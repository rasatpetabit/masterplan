...
- Coverage gaps closed: promote-without-marker, wave-less `active_run` throw (unit + CLI), no-op/invalid-status `mark-task`, `backfill` leftover-wave-less, cross-type `applyPlanIndex` match, and a 24-shape `parseState∘serializeState` round-trip fuzz. Every fix-test discriminates (fails on the pre-fix code). Next: Step 3 (`agents/*.md`).
