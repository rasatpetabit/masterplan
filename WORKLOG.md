...
- **(1) fixture-identifier scan** — the headline CUTOVER guard. `scanForRealIdentifiers()` deny-list over `test/fixtures/` *content*: `/srv/`, real `/home/<name>/` (synthetic `/home/user/` allowed), `\bpetabit` (the `\b` cannot fire inside `rasatpetabit` — the public marketplace id needs no allowlist clause), `coherent`/`FTCD`/`XCVR-`/`wbn`. Never matches bare 64-hex, so the index-staleness `sha256(plan.md)` content digest is a non-issue (resolves the deferred 64-hex question — it was never an identifier).
...
**Dropped (documented, no silent cap):** `router-size/prose` — a v7 heuristic with no v8 contract (the clean-core orchestrator has no enforced prompt-size budget). Also dropped the `-home-<name>-` slug heuristic: it false-positives on legitimate feature slugs (`home-page`, `home-screen`) — same substring-landmine class the advisor flagged for `petabit`; absolute `/srv/` + `/home/<name>/` cover the content-form path-leak vector.

Authoritative gate: `node --test test/*.test.mjs` → **232 pass / 0 fail / 0 skipped** (was 214; +18). Mutation check (proof the gates bite, not just pass): a doctored fixture string flagged `home-path,petabit-org,coherent-codename`; an injected `7.3.0` codex manifest flagged version drift; a synthetic `skills/plan/` flagged `shadows-builtin`. Orchestrator sole committer (CD-7). **Not pushed.**

**v8 execution track is now complete** (Phases 1–4 done: 6 doctor BUGs + 2 DECISIONS + CUTOVER sanitize + published-guards). **Still flagged for the user (a separate publish-hygiene call, intentionally NOT auto-rewritten):** WORKLOG.md + CHANGELOG.md carry `petabit-*` org/project references — the user's own org in the user's own repo. The fixture guard scopes to `test/fixtures/` only and does not touch these; whether they ship as-is is the owner's call.
