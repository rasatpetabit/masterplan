...
1. **Resume state via coordinator-bundle-resume.** On every execute-turn entry:
   ...
   **In-session mtime gating.** Maintain an orchestrator-memory cache `file_cache: {path → (mtime, content)}`. On a Step C entry within the **same session**, if a file's current mtime matches the cached mtime, reuse the cached content and skip the Read for that file. Cross-session entries (i.e. after a `ScheduleWakeup` resumption) start with an empty cache and always re-read. `state.yml` is **never** mtime-gated — always re-read live, since the orchestrator wrote it last and the user may have edited it between turns. Fail-safe: re-read on any doubt.
   ...
   - **Pending-gate resume.** If `pending_gate` is non-null, set `stop_reason: question` if it is missing or stale, then re-render that exact structured question before doing any new routing. Clear it only after CD-7's explicit selection-evidence rule is satisfied, applying the selected option, appending `gate_closed` to `events.jsonl`, and clearing `stop_reason` unless the chosen option itself closes the turn.
     - **Adversarial review plan gate carve-out.** When `pending_gate.id == adversarial_review_plan_pending` AND `state.adversarial_review_plan_pending_job.log_file` is set: skip the generic re-render and instead run the B3 step-5 completion check inline — `test -s <log_file>` (non-empty file = review output landed). If complete: read first 8192 chars of `log_file` as `review_output`; parse pass/fail via `/\b(critical|fatal|serious|blocking|fundamental|wrong assumption)\b/i`; append `{"event":"adversarial_review_complete","gate":"plan_approval","result":"<pass|fail>","findings_chars":<N>,"ts":"<now>"}`, clear `pending_gate` and `adversarial_review_plan_pending_job`, proceed to the B3 close-out gate. If NOT complete: re-schedule wakeup `ScheduleWakeup(delaySeconds=120, prompt="/masterplan --resume=<state-path>", reason="Checking adversarial review result for <slug> plan gate")`, set `stop_reason: scheduled_yield`, append `wakeup_scheduled` → CLOSE-TURN. If `adversarial_review_plan_pending_job` is absent (pre-fix bundle or skipped due to malformed response): fall through to the generic re-render (shows Poll now / Resume later AUQ).
   - **Background-dispatch resume.** If `background` is non-null, poll the recorded task before any new task dispatch. **Polling algorithm:** load deferred tools — `ToolSearch(query: "select:TaskGet,TaskOutput", max_results: 2)` — then call `TaskGet(id=background.agent_id)`. Status `running|queued` → still in-flight. Status `completed` → call `TaskOutput(id=background.agent_id)` to retrieve result. Status `failed|cancelled|error` → background_failed. Status `not_found` OR `TaskGet` unavailable OR `TaskGet` errors → **cross-session boundary**: `TaskGet` IDs are session-scoped; a ScheduleWakeup firing in a new session cannot resolve a prior session's task IDs. In this case: fall back to `test -s <background.output_path>` — non-empty file indicates available output written by the agent; empty/missing file means task result is unrecoverable (treat as background_failed). **Do NOT treat `not_found` as `failed`** — the agent may have completed successfully and written to `output_path` before the session ended. Do not redispatch the current task until this check resolves:
   ...
   - **Orphaned in-flight task resume (`wave-barrier-interrupted`).** After the Background-dispatch check above, if `background: null` AND any `tasks[*].status == "in_flight"`, scan `events.jsonl` for a `wave_task_completed` / `codex_task_completed` / `implementer_complete` event referencing each in-flight task index. Tasks with no matching completion event are **orphaned** — the session died while the wave-completion barrier was blocking (parallel Agent calls never returned). Detection algorithm: `orphaned_indices = [t.idx for t in state.tasks if t.status == "in_flight"]`; for each idx, grep `events.jsonl` for `wave_task_completed|codex_task_completed|implementer_complete` with a matching `task_idx` or `task` field. Any idx with no completion event is orphaned. If orphaned indices are found, append a `wave_barrier_interrupted` event listing them, then surface immediately via `AskUserQuestion`:
     - Question: `"Tasks [<idx-list>] were in-flight when the previous session ended (no completion event recorded — session likely died mid-wave). How to proceed?"`
     - Options:
       1. `"Re-dispatch orphaned tasks from scratch (Recommended)"` — set `status: pending` for each orphaned task in `state.yml`; continue Step C at the lowest-index orphan.
       2. `"Skip orphaned tasks — mark them complete without output"` — append `wave_task_skipped` for each orphaned task; set `status: complete` with digest `skipped`; continue Step C at the next pending task.
       3. `"Run orphaned tasks inline (ignore eligibility cache)"` — same as (1) but stamps `decision_source: user-override-manual` and routes inline.
       4. `"Abort — investigate first"` — set `stop_reason: question`, append `question_opened`, end turn without further state change.
     - **Skip condition:** if `background != null`, the Background-dispatch resume check above handles the in-flight case — this check does NOT run. If no task has `status: "in_flight"`, skip silently.
